...
#ifdef CONFIG_ENABLE_CHIP_TYPE_MSG21XXA
static s32 _DrvFwCtrlMsg21xxaUpdateFirmwareBySwId(u8 szFwData[][1024],
...
static void _DrvFwCtrlMsg22xxConvertFwDataTwoDimenToOneDimen(u8 szTwoDimenFwData[][1024],
		u8 *pOneDimenFwData)
{
	u32 i, j;
...
static s32 _DrvFwCtrlSelfParsePacket(u8 *pPacket, u16 nLength,
				     SelfTouchInfo_t *pInfo) /* for MSG21xxA/MSG22xx */
{
	u8 nCheckSum = 0;
	...
	if (g_ChipType == CHIP_TYPE_MSG22XX) {
		if (pPacket[0] == 0xA7 && pPacket[1] == 0x00 && pPacket[2] == 0x08 &&
		    pPacket[3] == PACKET_TYPE_ESD_CHECK_HW_RESET && pPacket[4] == 0xFF &&
		    pPacket[5] == 0xFF && pPacket[6] == 0xFF) {
			DBG(&g_I2cClient->dev,
			    "ESD HwReset check : g_IsUpdateFirmware=%d, g_IsHwResetByDriver=%d\n", g_IsUpdateFirmware,
			    g_IsHwResetByDriver);
	...
	if (g_GestureWakeupFlag == 1) {
		...
		if (g_ChipType == CHIP_TYPE_MSG22XX && pPacket[0] == 0xA7 && pPacket[1] == 0x00 &&
		    pPacket[2] == 0x06 && pPacket[3] == PACKET_TYPE_GESTURE_WAKEUP) {
	...
	if ((pPacket[nCheckSumIndex] == nCheckSum) && (pPacket[0] == 0x52)) {
		...
		if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) &&
		    (pPacket[4] == 0xFF) && (pPacket[6] == 0xFF)) {
...
#endif
		if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) &&
		    (pPacket[4] == 0xFF) && (pPacket[5] == 0xFF) && (pPacket[6] == 0xFF) &&
		    (pPacket[7] == 0xFF)) {
			pInfo->tPoint[0].nX = 0; /* final X coordinate */
			pInfo->tPoint[0].nY = 0; /* final Y coordinate */
...
static s32 _DrvFwCtrlMutualParsePacket(u8 *pPacket, u16 nLength,
				       MutualTouchInfo_t *pInfo) /* for MSG26xxM/MSG28xx */
{
	u32 i;
	u8 nCheckSum = 0;
	u32 nX = 0, nY = 0;
	...
	DBG(&g_I2cClient->dev, "received raw data from touch panel as following:\n");
	DBG(&g_I2cClient->dev, "pPacket[x]=%x %x %x %x %x %x %x %x %x\n",
	...
	if (g_GestureWakeupFlag == 1) {
		...
		DBG(&g_I2cClient->dev, "received raw data from touch panel as following:\n");
		DBG(&g_I2cClient->dev, "pPacket[x]=%x %x %x %x %x %x\n",
		pPacket[0], pPacket[1], pPacket[2], pPacket[3], pPacket[4], pPacket[5]);
	...
	if (IS_FIRMWARE_DATA_LOG_ENABLED) {
		...
		} else if (g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE && pPacket[0] != 0xA5 &&
			   pPacket[0] != 0xAB && (pPacket[0] != 0xA7 && pPacket[3] != PACKET_TYPE_TOOTH_PATTERN)) {
			DBG(&g_I2cClient->dev, "WRONG DEBUG MODE HEADER\n");
			return -EPERM;
		}
	} else {
	...
	if (pPacket[0] == 0x5A) {
		...
		if (((DemoHotKnotCmdRet_t *)pPacket)->nIdentify == DEMO_PD_PACKET_IDENTIFY) {
			...
			return -EPERM;    /* return 0 will run key procedure */
		}
#endif /* CONFIG_ENABLE_HOTKNOT */
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		if (g_TouchKObj != NULL) {
			...
			nRetVal = kobject_uevent_env(g_TouchKObj, KOBJ_CHANGE, pEnvp);
			DBG(&g_I2cClient->dev,
			    "kobject_uevent_env() STATUS=GET_DEBUG_MODE_PACKET, nRetVal = %d\n", nRetVal);
		}
...
static u16 _DrvFwCtrlMsg21xxaGetSwId(EmemType_e eEmemType)
{
	...
	DBG(&g_I2cClient->dev, "szRxData[0,1,2,3] = 0x%x,0x%x,0x%x,0x%x\n", szRxData[0],
	    szRxData[1], szRxData[2], szRxData[3]);
...
static s32 _DrvFwCtrlMsg26xxmUpdateFirmware(u8 szFwData[][1024], EmemType_e eEmemType)
{
	...
	g_IsUpdateFirmware =
		0x01; /* Set flag to 0x01 for indicating update firmware is processing */
	...
	g_IsUpdateFirmware =
		0x00; /* Set flag to 0x00 for indicating update firmware is finished */
...
static void _DrvFwCtrlMsg28xxConvertFwDataTwoDimenToOneDimen(u8 szTwoDimenFwData[][1024],
		u8 *pOneDimenFwData)
{
	u32 i, j;
...
static u32 _DrvFwCtrlMsg28xxCalculateCrc(u8 *pFwData, u32 nOffset, u32 nSize)
{
	...
	for (i = 0; i < nSize; i += 4) {
		nData = (pFwData[nOffset + i]) | (pFwData[nOffset + i + 1] << 8) |
			(pFwData[nOffset + i + 2] << 16) | (pFwData[nOffset + i + 3] << 24);
...
static void _DrvFwCtrlMsg28xxIspBurstWriteEFlashStart(u16 nStartAddr, u8 *pFirstData,
		u32 nBlockSize, u16 nPageNum, EmemType_e eEmemType)
{
...
static void _DrvFwCtrlMsg28xxGetSfrAddr3Value(void)
{
	...
	DBG(&g_I2cClient->dev,
	    "_gSFR_ADDR3_BYTE0_1_VALUE = 0x%4X, _gSFR_ADDR3_BYTE2_3_VALUE = 0x%4X\n",
...
static void _DrvFwCtrlMsg28xxProgramEmem(EmemType_e eEmemType)
{
	...
	if (eEmemType == EMEM_MAIN) { /* Program main block */
		...
		nPageNum = (MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE * 1024) /
			   MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE; /* 128*1024/128=1024 */
		...
		IicWriteData(SLAVE_I2C_ID_DBBUS, &szWriteData[0],
			     3 + nLength); /* Write the first two pages(page 0 & page 1) */
		...
		for (i = 2; i < nPageNum; i++) {
			if (i == 2) {
				...
				for (j = 0; j < nLength; j++) {
					szWriteData[3 + j] =
g_FwData[i / 8][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * (i - (8 * (i / 8))) + j];
				}
...
g_FwData[i / 8][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * (i - (8 * (i / 8))) + j];
		...
		for (i = 0; i < nPageNum; i++) {
			if (i == 0) {
				...
				_DrvFwCtrlMsg28xxIspBurstWriteEFlashStart(nIndex, &szFirstData[0],
						MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE * 1024, nPageNum, EMEM_MAIN);
				...
				nLength = MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE -
					  MSG28XX_EMEM_SIZE_BYTES_ONE_WORD; /* 124 = 128 - 4 */
#endif
	...
	g_FwData[i / 8][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * (i - (8 * (i / 8))) + j];
		...
		nPageNum = (MSG28XX_FIRMWARE_INFO_BLOCK_SIZE * 1024) /
			   MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE; /* 2*1024/128=16 */
...
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE];
				szFirstData[1] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 1];
				szFirstData[2] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 2];
				szFirstData[3] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 3];

				_DrvFwCtrlMsg28xxIspBurstWriteEFlashStart(nIndex, &szFirstData[0],
						MSG28XX_FIRMWARE_INFO_BLOCK_SIZE * 1024, nPageNum - 1, EMEM_INFO);
				...
				for (j = 0; j < (nLength / 8); j++) {
					for (k = 0; k < 8; k++) {
						szBufferData[k] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 4 + (8 * j) + k];
					}
...
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 4 + (8 * j) + k];
				...
				_DrvFwCtrlMsg28xxIspBurstWriteEFlashDoWrite(&szBufferData[0],
						(nLength % 8)); /* 124%8 = 4 */
				...
				for (j = 0; j < (nLength / 32); j++) {
					for (k = 0; k < 32; k++) {
						szBufferData[k] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 4 + (32 * j) + k];
					}
...
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 4 + (32 * j) + k];
				...
				for (j = 0; j < nLength; j++) {
					szBufferData[j] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 4 + j];
				}
#endif
			} else {
				nLength = MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE; /* 128 */
				...
				if (i < 8) { /* 1 < i < 8 */
					for (j = 0; j < (nLength / 8); j++) {
						for (k = 0; k < 8; k++) {
							szBufferData[k] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * i + (8 * j) + k];
						}
					...
					for (j = 0; j < (nLength / 8); j++) {
						for (k = 0; k < 8; k++) {
							szBufferData[k] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE + 1][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * (i - 8) + (8 * j) + k];
						}
				...
				if (i < 8) { /* 1 < i < 8 */
					for (j = 0; j < (nLength / 32); j++) {
						for (k = 0; k < 32; k++) {
							szBufferData[k] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * i + (32 * j) + k];
						}
					...
					for (j = 0; j < (nLength / 32); j++) {
						for (k = 0; k < 32; k++) {
							szBufferData[k] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE + 1][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * (i - 8) + (32 * j) + k];
						}
				...
				if (i < 8) { /* 1 < i < 8 */
					for (j = 0; j < nLength; j++) {
						szBufferData[j] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * i + j];
					}
				} else { /* i >= 8 */
					for (j = 0; j < nLength; j++) {
						szBufferData[j] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE + 1][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * (i - 8) + j];
					}
				}
#endif
			}
		...
		nWordNum = MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE /
			   MSG28XX_EMEM_SIZE_BYTES_ONE_WORD; /* 128/4=32 */
		nLength = MSG28XX_EMEM_SIZE_BYTES_ONE_WORD;

		for (i = 0; i < nWordNum; i++) {
			if (i == 0) {
				szFirstData[0] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE + 1][7 * MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE];
				szFirstData[1] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE + 1][7 * MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 1];
				szFirstData[2] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE + 1][7 * MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 2];
				szFirstData[3] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE + 1][7 * MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + 3];

				_DrvFwCtrlMsg28xxWriteEFlashStart(nIndex, &szFirstData[0], EMEM_INFO);
			} else {
				for (j = 0; j < nLength; j++) {
					szFirstData[j] =
g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE + 1][7 * MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE + (4 * i) + j];
				}
...
static u32 _DrvFwCtrlMsg28xxRetrieveFirmwareCrcFromEFlash(EmemType_e eEmemType)
{
	...
	if (eEmemType == EMEM_MAIN) { /* Read main block CRC(128KB-4) from main block */
		...
		nReadAddr = 0x7FFF;
		   ...
		   EMEM_INFO) { /* Read info block CRC(2KB-MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE-4) from info block */
...
static s32 _DrvFwCtrlUpdateFirmwareCash(u8 szFwData[][1024], EmemType_e eEmemType)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG21XXA) { /* (0x02) */
		...
		return _DrvFwCtrlMsg26xxmUpdateFirmware(szFwData, eEmemType);
		   ...
		   g_ChipType == CHIP_TYPE_MSG58XXA) { /* (0x85) || (0xBF) */
		...
		eVendorId = szFwData[129][1005] << 8 |
			    szFwData[129][1004]; /* Retrieve major from info block */
...
static s32 _DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG21XXA) {
		...
		eVendorId = g_FwData[129][1005] << 8 |
			    g_FwData[129][1004]; /* Retrieve major from info block */
		eSwId = _DrvFwCtrlMsg28xxGetSwId(EMEM_INFO);
	}
...
void DrvFwCtrlOpenGestureWakeup(u32 *pMode)
{
	...
	while (i < 5) {
		msleep(I2C_WRITE_COMMAND_DELAY_FOR_FIRMWARE); /* delay 20ms */
...
static void _DrvFwCtrlCoordinate(u8 *pRawData, u32 *pTranX, u32 *pTranY)
{
	...
	nX = (((pRawData[0] & 0xF0) << 4) |
	      pRawData[1]);         /* parse the packet to coordinate */
...
u32 DrvFwCtrlReadDQMemValue(u16 nAddr)
{
	if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
		...
		temp = rx_data[3] << 24 | rx_data[2] << 16 | rx_data[1] << 8 | rx_data[0];
		return temp;
	}
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
static u32 _DrvFwCtrlMsg22xxRetrieveFrimwareCrcFromBinFile(u8 szTmpBuf[],
		EmemType_e eEmemType)
{
	u32 nRetVal = 0;
...
static s32 _DrvFwCtrlMsg22xxUpdateFirmwareBySwId(void)
{
	...
	if (_gIsUpdateInfoBlockFirst == 1) {
		if ((g_IsUpdateFirmware & 0x10) == 0x10) {
			...
			nCrcInfoA = _DrvFwCtrlMsg22xxRetrieveFrimwareCrcFromBinFile(_gOneDimenFwData,
									EMEM_INFO);
			...
			if (nCrcInfoA == nCrcInfoB) {
				...
				nCrcMainA = _DrvFwCtrlMsg22xxRetrieveFrimwareCrcFromBinFile(_gOneDimenFwData,
									EMEM_MAIN);
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
	...
	DBG(&g_I2cClient->dev,
	    "*** sizeof(_gSwIdData)=%d, sizeof(SwIdData_t)=%d, nSwIdListNum=%d ***\n",
	...
	if (nCrcMainA == nCrcMainB &&
	    nCrcInfoA == nCrcInfoB) { /* Case 1. Main Block:OK, Info Block:OK */
		eSwId = _DrvFwCtrlMsg22xxGetSwId(EMEM_MAIN);
		...
		for (j = 0; j < nSwIdListNum; j++) {
			DBG(&g_I2cClient->dev, "_gSwIdData[%d].nSwId = 0x%x\n", j,
			    _gSwIdData[j].nSwId); /* TODO : add for debug */

			if (eSwId == _gSwIdData[j].nSwId) {
				nUpdateBinMajor = (*(_gSwIdData[j].pUpdateBin + 0xBFF5)) << 8 | (*
						  (_gSwIdData[j].pUpdateBin + 0xBFF4));
				nUpdateBinMinor = (*(_gSwIdData[j].pUpdateBin + 0xBFF7)) << 8 | (*
		...
		DBG(&g_I2cClient->dev,
		    ...
		    nMajor, nMinor, nUpdateBinMajor, nUpdateBinMinor);
		...
		if (nUpdateBinMinor > nMinor) {
			if (nIsSwIdValid == 1) {
				...
				for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE + 1); i++) {
					if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) { /* i < 48 */
			_DrvFwCtrlStoreFirmwareData((_gSwIdData[nFinalIndex].pUpdateBin + i * 1024), 1024);
					} else { /* i == 48 */
			_DrvFwCtrlStoreFirmwareData((_gSwIdData[nFinalIndex].pUpdateBin + i * 1024), 512);
					}
				}
			} else {
				DBG(&g_I2cClient->dev, "eSwId = 0x%x is an undefined SW ID.\n", eSwId);
	...
	} else if (nCrcMainA == nCrcMainB &&
		   nCrcInfoA != nCrcInfoB) { /* Case 2. Main Block:OK, Info Block:FAIL */
		eSwId = _DrvFwCtrlMsg22xxGetSwId(EMEM_MAIN);
		...
		if (eSwId < MSG22XX_SW_ID_UNDEFINED && eSwId != 0x0000 && eSwId != 0xFFFF) {
		   ...
		   nCrcInfoA == nCrcInfoB) { /* Case 3. Main Block:FAIL, Info Block:OK */
			...
			if (i == 31) {
				...
				for (j = 0; j < 4; j++) {
					...
					DBG(&g_I2cClient->dev, "((nCrcTemp>>(8*%d)) & 0xFF)=%x\n", j,
					    ((nCrcTemp >> (8 * j)) & 0xFF)); /* add for debug */
...
void _DrvFwCtrlMsg21xxaCheckFirmwareUpdateBySwId(void)
{
	...
	u8 *pVersion = NULL;
	Msg21xxaSwId_e eMainSwId = MSG21XXA_SW_ID_UNDEFINED, eInfoSwId = MSG21XXA_SW_ID_UNDEFINED,
	...
	if (nCrcMainA == nCrcMainB) {
		...
		if (eSwId == MSG21XXA_SW_ID_XXXX) {
			...
			nUpdateBinMajor = msg21xxa_xxxx_update_bin[31][0x34F] << 8 |
					  msg21xxa_xxxx_update_bin[31][0x34E];
			nUpdateBinMinor = msg21xxa_xxxx_update_bin[31][0x351] << 8 |
					  msg21xxa_xxxx_update_bin[31][0x350];
#else /* By one dimensional array */
			nUpdateBinMajor = msg21xxa_xxxx_update_bin[0x7F4F] << 8 |
					  msg21xxa_xxxx_update_bin[0x7F4E];
			nUpdateBinMinor = msg21xxa_xxxx_update_bin[0x7F51] << 8 |
					  msg21xxa_xxxx_update_bin[0x7F50];
#endif
		} else if (eSwId == MSG21XXA_SW_ID_YYYY) {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY /* By two dimensional array */
			nUpdateBinMajor = msg21xxa_yyyy_update_bin[31][0x34F] << 8 |
					  msg21xxa_yyyy_update_bin[31][0x34E];
			nUpdateBinMinor = msg21xxa_yyyy_update_bin[31][0x351] << 8 |
					  msg21xxa_yyyy_update_bin[31][0x350];
#else /* By one dimensional array */
			nUpdateBinMajor = msg21xxa_yyyy_update_bin[0x7F4F] << 8 |
					  msg21xxa_yyyy_update_bin[0x7F4E];
			nUpdateBinMinor = msg21xxa_yyyy_update_bin[0x7F51] << 8 |
					  msg21xxa_yyyy_update_bin[0x7F50];
#endif
...
#ifdef CONFIG_ENABLE_CHIP_TYPE_MSG26XXM
static u32 _DrvFwCtrlMsg26xxmCalculateFirmwareCrcFromEFlash(EmemType_e eEmemType,
		u8 nIsNeedResetHW)
{
	u32 nRetVal = 0;
	u16 nRegData = 0;
	...
	if (eEmemType == EMEM_MAIN) { /* Read calculated main block CRC(32K-8) from register */
		...
		DBG(&g_I2cClient->dev, "Main Block CRC = 0x%x\n", nRetVal);
		   ...
		   EMEM_INFO) { /* Read calculated info block CRC(8K) from register */
...
static u32 _DrvFwCtrlMsg26xxmRetrieveFrimwareCrcFromBinFile(u8 szTmpBuf[][1024],
		EmemType_e eEmemType)
{
	u32 nRetVal = 0;
...
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY /* By two dimensional array */
			nUpdateBinMajor = msg26xxm_xxxx_update_bin[0][0x2B] << 8 |
					  msg26xxm_xxxx_update_bin[0][0x2A];
			nUpdateBinMinor = msg26xxm_xxxx_update_bin[0][0x2D] << 8 |
					  msg26xxm_xxxx_update_bin[0][0x2C];
#else /* By one dimensional array */
			nUpdateBinMajor = msg26xxm_xxxx_update_bin[0x002B] << 8 |
					  msg26xxm_xxxx_update_bin[0x002A];
			nUpdateBinMinor = msg26xxm_xxxx_update_bin[0x002D] << 8 |
					  msg26xxm_xxxx_update_bin[0x002C];
#endif
		} else if (eSwId == MSG26XXM_SW_ID_YYYY) {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY /* By two dimensional array */
			nUpdateBinMajor = msg26xxm_yyyy_update_bin[0][0x2B] << 8 |
					  msg26xxm_yyyy_update_bin[0][0x2A];
			nUpdateBinMinor = msg26xxm_yyyy_update_bin[0][0x2D] << 8 |
					  msg26xxm_yyyy_update_bin[0][0x2C];
#else /* By one dimensional array */
			nUpdateBinMajor = msg26xxm_yyyy_update_bin[0x002B] << 8 |
					  msg26xxm_yyyy_update_bin[0x002A];
			nUpdateBinMinor = msg26xxm_yyyy_update_bin[0x002D] << 8 |
...
void _DrvFwCtrlMsg28xxCheckFirmwareUpdateBySwId(void)
{
	...
	if (nCrcMainA == nCrcMainB) {
		...
		for (j = 0; j < nSwIdListNum; j++) {
			...
			if (eSwId == _gSwIdData[j].nSwId) {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY /* By two dimensional array */
				nUpdateBinMajor = ((*(_gSwIdData[j].pUpdateBin + 127))[1013]) << 8 | ((*
						  (_gSwIdData[j].pUpdateBin + 127))[1012]);
				nUpdateBinMinor = ((*(_gSwIdData[j].pUpdateBin + 127))[1015]) << 8 | ((*
						  (_gSwIdData[j].pUpdateBin + 127))[1014]);
#else /* By one dimensional array */
				nUpdateBinMajor = (*(_gSwIdData[j].pUpdateBin + 0x1FFF5)) << 8 | (*
						  (_gSwIdData[j].pUpdateBin + 0x1FFF4));
				nUpdateBinMinor = (*(_gSwIdData[j].pUpdateBin + 0x1FFF7)) << 8 | (*
		...
		global_eSwId = eSwId;
		global_nMajor = nMajor;
		global_nMinor = nMinor;
		...
		if (eSwId < MSG28XX_SW_ID_UNDEFINED && eSwId != 0x0000 && eSwId != 0xFFFF) {
			g_FwDataCount =
				0; /* Reset g_FwDataCount to 0 after copying update firmware data to temp buffer */
			...
			if (i == 31) {
				...
				for (j = 0; j < 4; j++) {
					...
					DBG(&g_I2cClient->dev, "((nCrcTemp>>(8*%d)) & 0xFF)=%x\n", j,
					    ((nCrcTemp >> (8 * j)) & 0xFF)); /* add for debug */
					DBG(&g_I2cClient->dev, "szFwData[%d][%d]=%x\n", i,
					    (1020 + j), szFwData[i][1020 + j]); /* add for debug */
				}
			} else {
				for (j = 0; j < 1024; j++) {
					nCrcMain = DrvCommonCrcGetValue(szFwData[i][j], nCrcMain);
		...
		while (nRemainSize > 0) {
			...
			if (g_Msg22xxChipRevision >= CHIP_TYPE_MSG22XX_REVISION_U05) {
				...
				for (i = 0; i < nBlockSize; i++) {
					for (j = 0; j < 3; j++) {
						szTxData[3 + (i * 3) + j] =
_gOneDimenFwData[(MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE * 1024) + (index * nSizePerWrite) + i];
					}
					nSize = nSize + 3;
				}
#elif defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_B)
...
#else /* CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_C */
				for (i = 0; i < nBlockSize; i++) {
					szTxData[3 + i] = _gOneDimenFwData[(MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE * 1024) +
...
void DrvFwCtrlOptimizeCurrentConsumption(void)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG22XX) {
		...
		IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0],
			     3 + 24); /* Write 0x11 for reg 0x1150~0x115B */
		...
		IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0],
			     3 + 6); /* Write 0xFF for reg 0x115C~0x115E */
		...
		IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0],
			     3 + 8); /* Write 0xFF for reg 0x1510~0x1513 */
		...
		IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0],
			     3 + 16); /* Write 0x00 for reg 0x1514~0x151B */
		...
		IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0],
			     3 + 8); /* Write 0xFF for reg 0x2120~0x2123 */
		...
		IicWriteData(SLAVE_I2C_ID_DBBUS, &szTxData[0],
			     3 + 32); /* Write 0xFF for reg 0x2110~0x211F */
...
void DrvFwCtrlGetCustomerFirmwareVersionByDbBus(EmemType_e eEmemType, u16 *pMajor,
		u16 *pMinor, u8 **ppVersion) /* support MSG28xx only */
{
	u16 nReadAddr = 0;
	u8  szTmpData[4] = {0};
	...
	if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
		...
		*pMinor |= szTmpData[2];
		...
		snprintf(*ppVersion, sizeof(u8) * 11, "%05d.%05d", *pMajor, *pMinor);
...
void DrvFwCtrlGetCustomerFirmwareVersion(u16 *pMajor, u16 *pMinor, u8 **ppVersion)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG21XX) {
		...
		*pMinor = (szRxData[3] << 8) + szRxData[2];
		...
		msleep(I2C_SMBUS_READ_COMMAND_DELAY_FOR_PLATFORM);
		IicReadData(SLAVE_I2C_ID_DWI2C, &szRxData[0], 4);
	...
	snprintf(*ppVersion, sizeof(u8) * 16, "%05d.%05d", *pMajor, *pMinor);
}
...
void DrvFwCtrlGetPlatformFirmwareVersion(u8 **ppVersion)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG22XX) {
		...
		for (i = 0; i < 3; i++) {
			...
			szRxData[i*4+0], szRxData[i*4+0]); // add for debug */
			/* DBG(&g_I2cClient->dev, "szRxData[%d] = 0x%x , %c\n", i*4+1,
			szRxData[i*4+1], szRxData[i*4+1]); // add for debug */

			szRxData[i * 4 + 2] = (nRegData2 & 0xFF);
			szRxData[i * 4 + 3] = ((nRegData2 >> 8) & 0xFF);

			/* DBG(&g_I2cClient->dev, "szRxData[%d] = 0x%x , %c\n", i*4+2,
			szRxData[i*4+2], szRxData[i*4+2]); // add for debug */
			/* DBG(&g_I2cClient->dev, "szRxData[%d] = 0x%x , %c\n", i*4+3,
			szRxData[i*4+3], szRxData[i*4+3]); // add for debug */
		}
		...
		snprintf(*ppVersion, sizeof(u8) * 10, "%c%c%c%c%c%c%c%c%c%c", szRxData[2], szRxData[3], szRxData[4],
			szRxData[5], szRxData[6], szRxData[7], szRxData[8], szRxData[9], szRxData[10],
			szRxData[11]);
		...
		snprintf(*ppVersion, sizeof(u8) * 16, "%.16s", szRxData);
		...
		DBG(&g_I2cClient->dev, "szRxData[4] = 0x%x , %c\n", szRxData[4],
		    szRxData[4]); /* add for debug */
		DBG(&g_I2cClient->dev, "szRxData[5] = 0x%x , %c\n", szRxData[5],
		    szRxData[5]); /* add for debug */
		DBG(&g_I2cClient->dev, "szRxData[6] = 0x%x , %c\n", szRxData[6],
		    szRxData[6]); /* add for debug */
		DBG(&g_I2cClient->dev, "szRxData[7] = 0x%x , %c\n", szRxData[7],
		    szRxData[7]); /* add for debug */
		DBG(&g_I2cClient->dev, "szRxData[8] = 0x%x , %c\n", szRxData[8],
		    szRxData[8]); /* add for debug */
		DBG(&g_I2cClient->dev, "szRxData[9] = 0x%x , %c\n", szRxData[9],
		    szRxData[9]); /* add for debug */

		if (*ppVersion == NULL) {
			*ppVersion = kzalloc(sizeof(u8) * 10, GFP_KERNEL);
		}

		snprintf(*ppVersion, sizeof(u8) * 10, "%.10s", szRxData);
	} else {
		if (*ppVersion == NULL) {
			*ppVersion = kzalloc(sizeof(u8) * 10, GFP_KERNEL);
		}

		snprintf(*ppVersion, sizeof(u8) * 10, "%s", "N/A");
	}
...
void DrvFwCtrlMutualGetFirmwareInfo(MutualFirmwareInfo_t
				    *pInfo) /* for MSG26xxM/MSG28xx */
{
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM) {
		...
		if (pInfo->nLogModePacketHeader == 0xA5) {
			...
			if (pInfo->nMy != 0 && pInfo->nMx != 0) {
				...
				pInfo->nLogModePacketLength = 1 + 1 + 1 + 1 + 10 * 3 + pInfo->nMy * pInfo->nMx * 2 +
							      pInfo->nMy * 2 + pInfo->nMx * 2 + 2 * 2 + 8 * 2 + 1;
			} else {
				DBG(&g_I2cClient->dev,
				    "Failed to retrieve channel number or subframe number for debug mode packet 0xAB.\n");
			}
		} else if (pInfo->nLogModePacketHeader == 0xA7 &&
			   pInfo->nType == PACKET_TYPE_TOOTH_PATTERN) {
			if (pInfo->nMy != 0 && pInfo->nMx != 0 && pInfo->nSd != 0 && pInfo->nSs != 0) {
				/* for parsing debug mode packet 0xA7 */
				pInfo->nLogModePacketLength = 1 + 1 + 1 + 1 + 1 + 10 * 3 + pInfo->nMy * pInfo->nMx * 2 +
							      pInfo->nSd * 2 + pInfo->nSs * 2 + 10 * 2 + 1;
			} else {
				DBG(&g_I2cClient->dev,
...
void DrvFwCtrlRestoreFirmwareModeToLogDataMode(void)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX) {
		...
		if (g_IsSwitchModeByAPK == 1) {
			...
			if (g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE &&
			    DrvFwCtrlGetFirmwareMode() == FIRMWARE_MODE_DEMO_MODE) {
				g_FirmwareMode = DrvFwCtrlChangeFirmwareMode(FIRMWARE_MODE_DEBUG_MODE);
			} else {
				DBG(&g_I2cClient->dev, "firmware mode is not restored\n");
			}
			...
			if (tInfo.nTouchKeyCode != 0) {
				...
				for (i = 0; i < MAX_TOUCH_NUM; i++) {
					if (tInfo.nFingerNum != 0) {
						if (_gCurrPress[i]) {
							input_report_key(g_InputDevice, BTN_TOUCH, 1);
					DrvPlatformLyrFingerTouchPressed(tInfo.tPoint[i].nX, tInfo.tPoint[i].nY, 1, i);
							input_report_key(g_InputDevice, BTN_TOOL_FINGER, 1);
						} else if (_gCurrPress[i] == 0 && _gPriorPress[i] == 1) {
							DrvPlatformLyrFingerTouchReleased(0, 0, i);
						}
					}
					_gPriorPress[i] = _gCurrPress[i];
...
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
			DBG(&g_I2cClient->dev, "nKeyCode=%x, nLastKeyCode=%x, g_TpVirtualKey[%d]=%d\n",
			    tInfo.nKeyCode, nLastKeyCode, tInfo.nKeyCode, g_TpVirtualKey[tInfo.nKeyCode]);
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
#ifdef CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD
			if (tpd_dts_data.use_tpd_button) {
				DBG(&g_I2cClient->dev,
				    "nKeyCode=%x, nLastKeyCode=%x, tpd_key_local[%d]=%d\n", tInfo.nKeyCode,
				    nLastKeyCode, tInfo.nKeyCode, tpd_dts_data.tpd_key_local[tInfo.nKeyCode]);
			}
#else
			DBG(&g_I2cClient->dev, "nKeyCode=%x, nLastKeyCode=%x, g_TpVirtualKey[%d]=%d\n",
			    tInfo.nKeyCode, nLastKeyCode, tInfo.nKeyCode, g_TpVirtualKey[tInfo.nKeyCode]);
		...
		if (tInfo.nCount > 0) {        /* point touch pressed */
			...
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
				DBG(&g_I2cClient->dev, "_gPreviousTouch[%d]=%d, _gCurrentTouch[%d]=%d\n", i,
				    _gPreviousTouch[i], i, _gCurrentTouch[i]); /* TODO : add for debug */
