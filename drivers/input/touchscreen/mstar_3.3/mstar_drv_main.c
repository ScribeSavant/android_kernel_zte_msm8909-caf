...
ssize_t DrvMainProcfsChipTypeRead(struct file *pFile, char __user *pBuffer, size_t nCount,
				  loff_t *pPos)
{
	u32 nLength = 0;
	...
	nLength = snprintf(pBuffer, nCount, "%d", g_ChipType);
...
ssize_t DrvMainProcfsChipTypeWrite(struct file *pFile, const char __user *pBuffer,
...
ssize_t DrvMainProcfsFirmwareDataWrite(struct file *pFile, const char __user *pBuffer,
				       size_t nCount, loff_t *pPos)
{
...
ssize_t DrvMainProcfsFirmwareUpdateRead(struct file *pFile, char __user *pBuffer,
					size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	nLength = snprintf(pBuffer, nCount, "%d", _gIsUpdateComplete);
...
ssize_t DrvMainProcfsFirmwareUpdateWrite(struct file *pFile, const char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
...
ssize_t DrvMainProcfsCustomerFirmwareVersionRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	nLength = snprintf(pBuffer, nCount, "%s\n", _gFwVersion);
...
ssize_t DrvMainProcfsPlatformFirmwareVersionRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
ssize_t DrvMainProcfsDeviceDriverVersionRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	nLength = snprintf(pBuffer, nCount, "%s", DEVICE_DRIVER_RELEASE_VERSION);
...
ssize_t DrvMainProcfsSdCardFirmwareUpdateRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u16 nMajor = 0, nMinor = 0;
	u32 nLength = 0;
		...
		const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	...
	if (pBuffer != NULL) {
		pValid = strnstr(pBuffer, ".bin", nCount);
...
ssize_t DrvMainProcfsSeLinuxLimitFirmwareUpdateRead(struct file *pFile,
...
ssize_t DrvMainProcfsForceFirmwareUpdateRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	nLength = snprintf(pBuffer, nCount, "%d", _gFeatureSupportStatus);
...
ssize_t DrvMainProcfsFirmwareDebugRead(struct file *pFile, char __user *pBuffer,
				       size_t nCount, loff_t *pPos)
{
	u32 i, nLength = 0;
	u8 nBank, nAddr;
...
ssize_t DrvMainProcfsFirmwareDebugWrite(struct file *pFile, const char __user *pBuffer,
					size_t nCount, loff_t *pPos)
{
	u32 i;
	char *pCh = NULL;
...
ssize_t DrvMainProcfsFirmwareSetDebugValueRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 i, nLength = 0;
	u8 nBank, nAddr;
	...
	if (pBuffer != NULL) {
		...
		for (i = 0; i < _gDebugRegCount; i++) {
			...
			DBG(&g_I2cClient->dev, "_gDebugReg[%d] = 0x%04X, _gDebugRegValue[%d] = 0x%04X\n", i,
			    _gDebugReg[i], i, _gDebugRegValue[i]);  /* add for debug */
		}
...
ssize_t DrvMainProcfsFirmwareSmBusDebugRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 i = 0, nLength = 0;
...
ssize_t DrvMainProcfsFirmwareSmBusDebugWrite(struct file *pFile,
		const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u32 i, j;
	char szCmdType[5] = {0};
	...
	if (pBuffer != NULL) {
		...
		while ((pCh = strsep((char **)&pStr, " ,")) && (j < MAX_DEBUG_COMMAND_ARGUMENT_NUM)) {
			...
			if (strcmp(pCh, "w") == 0 || strcmp(pCh, "r") == 0) {
				...
				if (sscanf(pCh, "%d", &_gDebugReadDataSize) != 1)
					pr_info("%s %d sscanf failed!!\n", __func__, __LINE__);
...
ssize_t DrvMainProcfsFirmwareSetDQMemValueRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 i, nLength = 0;
	u8 nBank, nAddr;
...
ssize_t DrvMainProcfsFirmwareSetDQMemValueWrite(struct file *pFile,
		const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u32 i, j, k;
	char *pCh = NULL;
...
ssize_t DrvMainProcfsMpTestRead(struct file *pFile, char __user *pBuffer, size_t nCount,
				loff_t *pPos)
{
	u32 nLength = 0;
	...
	nLength = snprintf(pBuffer, nCount, "%d", DrvIcFwLyrGetMpTestResult());
...
ssize_t DrvMainProcfsMpTestLogWrite(struct file *pFile, const char __user *pBuffer,
				    size_t nCount, loff_t *pPos)
{
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
ssize_t DrvMainProcfsMpTestFailChannelRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
				     ...
				     size_t nCount, loff_t *pPos)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX ||
		...
		nLength = snprintf(pBuffer, nCount, "%d,%d,%d", g_TestScopeInfo.nMx, g_TestScopeInfo.nMy,
				  g_TestScopeInfo.nKeyNum);
	}
#endif /* CONFIG_ENABLE_CHIP_TYPE_MSG26XXM || CONFIG_ENABLE_CHIP_TYPE_MSG28XX */
...
ssize_t DrvMainProcfsMpTestScopeWrite(struct file *pFile, const char __user *pBuffer,
				      size_t nCount, loff_t *pPos)
{
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
ssize_t DrvMainProcfsMpTestLogAllRead(struct file *pFile, char __user *pBuffer,
				      size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
ssize_t DrvMainProcfsMpTestLogAllWrite(struct file *pFile, const char __user *pBuffer,
				       size_t nCount, loff_t *pPos)
{
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
				      ...
				      size_t nCount, loff_t *pPos)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM) {
		...
		nLength = snprintf(pBuffer, nCount, "%x", g_FirmwareMode);
		...
		DBG(&g_I2cClient->dev, "%s() firmware mode = 0x%x\n", __func__,
		    g_MutualFirmwareInfo.nFirmwareMode);

		nLength = snprintf(pBuffer, nCount, "%x", g_MutualFirmwareInfo.nFirmwareMode);
		...
		nLength = snprintf(pBuffer, nCount, "%x,%d", g_SelfFirmwareInfo.nFirmwareMode,
...
ssize_t DrvMainProcfsFirmwareModeWrite(struct file *pFile, const char __user *pBuffer,
				       size_t nCount, loff_t *pPos)
{
	u32 nMode;
	...
	if (pBuffer != NULL) {
		...
		if (nMode == FIRMWARE_MODE_DEMO_MODE) { /* demo mode */
			...
			_gnDebugLogTimesStamp =
				0; /* Set _gnDebugLogTimesStamp for filter duplicate packet on MTPTool APK */
...
ssize_t DrvMainProcfsFirmwareSensorRead(struct file *pFile, char __user *pBuffer,
					size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX ||
		...
		if (g_MutualFirmwareInfo.nLogModePacketHeader == 0xA5 ||
			...
			nLength = snprintf(pBuffer, nCount, "%d,%d",
				g_MutualFirmwareInfo.nMx, g_MutualFirmwareInfo.nMy);
		} else if (g_MutualFirmwareInfo.nLogModePacketHeader == 0xA7) {
			nLength = snprintf(pBuffer, nCount, "%d,%d,%d,%d",
				g_MutualFirmwareInfo.nMx, g_MutualFirmwareInfo.nMy,
				g_MutualFirmwareInfo.nSs, g_MutualFirmwareInfo.nSd);
		} else {
		...
		nLength = snprintf(pBuffer, nCount, "%d", g_SelfFirmwareInfo.nLogModePacketLength);
	}
...
ssize_t DrvMainProcfsFirmwareSensorWrite(struct file *pFile, const char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
ssize_t DrvMainProcfsFirmwarePacketHeaderRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX ||
	    g_ChipType == CHIP_TYPE_MSG58XXA) {
		nLength = snprintf(pBuffer, nCount, "%d", g_MutualFirmwareInfo.nLogModePacketHeader);
	} else if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX) {
		nLength = snprintf(pBuffer, nCount, "%d", g_SelfFirmwareInfo.nLogModePacketHeader);
	}
...
ssize_t DrvMainKObjectPacketShow(struct kobject *pKObj, struct kobj_attribute *pAttr,
				 char *pBuf)
{
	u32 nLength = 0;
	...
	if (strcmp(pAttr->attr.name, "packet") == 0) {
		if (g_FirmwareMode == FIRMWARE_MODE_DEMO_MODE) {
			if (g_DemoModePacket != NULL) {
				DBG(&g_I2cClient->dev, "Firmware Mode=%x, Packet[X]=%x, %x, %x, %x, %x, %x\n",
			...
			if (g_LogModePacket != NULL) {
				...
				if ((g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX
					...
					DBG(&g_I2cClient->dev, "CURRENT MODE IS NOT DEBUG MODE/WRONG DEBUG\n");
				}
			} else {
...
ssize_t DrvMainKObjectPacketStore(struct kobject *pKObj, struct kobj_attribute *pAttr,
...
static struct kobj_attribute packet_attr = __ATTR(packet, 0664, DrvMainKObjectPacketShow,
		DrvMainKObjectPacketStore);
...
ssize_t DrvMainProcfsQueryFeatureSupportStatusRead(struct file *pFile,
		char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
ssize_t DrvMainProcfsQueryFeatureSupportStatusWrite(struct file *pFile,
		const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u32 nFeature;
...
ssize_t DrvMainProcfsChangeFeatureSupportStatusRead(struct file *pFile,
		char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
ssize_t DrvMainProcfsChangeFeatureSupportStatusWrite(struct file *pFile,
		const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
	u32 i;
...
ssize_t DrvMainProcfsGestureWakeupModeRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	nLength = snprintf(pBuffer, nCount, "%x,%x", g_GestureWakeupMode[0], g_GestureWakeupMode[1]);
#else
	DBG(&g_I2cClient->dev, "g_GestureWakeupMode = 0x%x\n", g_GestureWakeupMode[0]);

	nLength = snprintf(pBuffer, nCount, "%x", g_GestureWakeupMode[0]);
...
#else
		if (sscanf(pBuffer, "%x", &nWakeupMode[0]) != 1)
			pr_info("%s %d sscanf failed!!\n", __func__, __LINE__);
...
ssize_t DrvMainProcfsGestureDebugModeRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	nLength = snprintf(pBuffer, nCount, "%d", g_GestureDebugMode);
...
ssize_t DrvMainProcfsGestureDebugModeWrite(struct file *pFile, const char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u8 ucGestureMode[2];
	u8 i;
...
ssize_t DrvMainProcfsGestureInforModeRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	...
	if (_gLogGestureInforType ==
	    FIRMWARE_GESTURE_INFORMATION_MODE_A) { /* FIRMWARE_GESTURE_INFORMATION_MODE_A */
		for (i = 0; i < 2; i++) {
		...
		for (i = 2; i < 8; i++) {
		   ...
		   FIRMWARE_GESTURE_INFORMATION_MODE_B) { /* FIRMWARE_GESTURE_INFORMATION_MODE_B */
		for (i = 0; i < 2; i++) {
			szLogGestureInfo[_gLogGestureCount] = g_LogGestureInfor[4 + i];
			_gLogGestureCount++;
		}
		...
		for (i = 0; i < g_LogGestureInfor[5] * 2 ; i++) {
		   ...
		   FIRMWARE_GESTURE_INFORMATION_MODE_C) { /* FIRMWARE_GESTURE_INFORMATION_MODE_C */
		for (i = 0; i < 6; i++) {
...
ssize_t DrvMainProcfsGestureInforModeWrite(struct file *pFile, const char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nMode;
				    ...
				    size_t nCount, loff_t *pPos)
{
	...
	nLength = snprintf(pBuffer, nCount, "%d,%d", g_InterruptReportRate, g_ValidTouchReportRate);
...
ssize_t DrvMainProcfsReportRateWrite(struct file *pFile, const char __user *pBuffer,
				     size_t nCount, loff_t *pPos)
{
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);

	if (pBuffer != NULL) {
		if (sscanf(pBuffer, "%d", &g_IsEnableReportRate) != 1)
			pr_info("%s %d sscanf failed!!\n", __func__, __LINE__);
				   ...
				   size_t nCount, loff_t *pPos)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
		...
		nLength = snprintf(pBuffer, nCount, "%x", nGloveMode);
	}
...
ssize_t DrvMainProcfsGloveModeWrite(struct file *pFile, const char __user *pBuffer,
				    size_t nCount, loff_t *pPos)
{
	u32 nGloveMode = 0;
	u32 i = 0;
...
ssize_t DrvMainProcfsOpenGloveModeRead(struct file *pFile, char __user *pBuffer,
				       size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
ssize_t DrvMainProcfsCloseGloveModeRead(struct file *pFile, char __user *pBuffer,
					size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
...
ssize_t DrvMainProcfsLeatherSheathModeRead(struct file *pFile, char __user *pBuffer,
		size_t nCount, loff_t *pPos)
{
	u32 nLength = 0;
	...
	if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
		...
		nLength = snprintf(pBuffer, nCount, "%x", nLeatherSheathMode);
	}
...
ssize_t DrvMainProcfsLeatherSheathModeWrite(struct file *pFile,
		const char __user *pBuffer, size_t nCount, loff_t *pPos)
{
...
static ssize_t hotknot_value_show(struct kobject *kobj, struct kobj_attribute *attr,
				  char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", hotknot_value);
}

static ssize_t hotknot_value_store(struct kobject *kobj, struct kobj_attribute *attr,
				   const char *buf, size_t count)
{
	if (sscanf(buf, "%d", &hotknot_value) != 1)
		pr_info("%s %d sscanf failed!!\n", __func__, __LINE__);

	return count;
}

static struct kobj_attribute hotknot_value_attr = __ATTR(hotknot_value, 0666,
		hotknot_value_show, hotknot_value_store);
...
s32 DrvMainTouchDeviceInitialize(void)
{
	...
	g_ChipType =
		DrvIcFwLyrGetChipType(); /* Try to get chip type by SLAVE_I2C_ID_DBBUS(0x62) firstly. */

	if (g_ChipType ==
	    0) { /* If failed, try to get chip type by SLAVE_I2C_ID_DBBUS(0x59) again. */
...
static s32 _DrvMainCreateProcfsDirEntry(void)
{
	...
	_gProcMsTouchScreenMsg20xxEntry = proc_mkdir(PROC_NODE_MS_TOUCHSCREEN_MSG20XX,
					  _gProcClassEntry);

	_gProcDeviceEntry = proc_mkdir(PROC_NODE_DEVICE, _gProcMsTouchScreenMsg20xxEntry);

	_gProcChipTypeEntry = proc_create(PROC_NODE_CHIP_TYPE, PROCFS_AUTHORITY,
	...
	_gProcFirmwareDataEntry = proc_create(PROC_NODE_FIRMWARE_DATA, PROCFS_AUTHORITY,
					      _gProcDeviceEntry, &_gProcFirmwareData);
	if (_gProcFirmwareDataEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_DATA);
	} else {
	...
	_gProcApkFirmwareUpdateEntry = proc_create(PROC_NODE_FIRMWARE_UPDATE, PROCFS_AUTHORITY,
				       _gProcDeviceEntry, &_gProcApkFirmwareUpdate);
	if (_gProcApkFirmwareUpdateEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_UPDATE);
	} else {
	...
	_gProcCustomerFirmwareVersionEntry = proc_create(PROC_NODE_CUSTOMER_FIRMWARE_VERSION,
					     PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcCustomerFirmwareVersion);
	if (_gProcCustomerFirmwareVersionEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_CUSTOMER_FIRMWARE_VERSION);
	} else {
	...
	_gProcPlatformFirmwareVersionEntry = proc_create(PROC_NODE_PLATFORM_FIRMWARE_VERSION,
					     PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcPlatformFirmwareVersion);
	if (_gProcPlatformFirmwareVersionEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_PLATFORM_FIRMWARE_VERSION);
	} else {
	...
	_gProcDeviceDriverVersionEntry = proc_create(PROC_NODE_DEVICE_DRIVER_VERSION,
					 PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcDeviceDriverVersion);
	if (_gProcDeviceDriverVersionEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_DEVICE_DRIVER_VERSION);
	} else {
	...
	_gProcSdCardFirmwareUpdateEntry = proc_create(PROC_NODE_SD_CARD_FIRMWARE_UPDATE,
					  PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcSdCardFirmwareUpdate);
	if (_gProcSdCardFirmwareUpdateEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_SD_CARD_FIRMWARE_UPDATE);
	} else {
	...
	_gProcFirmwareDebugEntry = proc_create(PROC_NODE_FIRMWARE_DEBUG, PROCFS_AUTHORITY,
					       _gProcDeviceEntry, &_gProcFirmwareDebug);
	if (_gProcFirmwareDebugEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_DEBUG);
	} else {
	...
	_gProcFirmwareSetDebugValueEntry = proc_create(PROC_NODE_FIRMWARE_SET_DEBUG_VALUE,
					   PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcFirmwareSetDebugValue);
	if (_gProcFirmwareSetDebugValueEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_SET_DEBUG_VALUE);
	} else {
	...
	_gProcFirmwareSmBusDebugEntry = proc_create(PROC_NODE_FIRMWARE_SMBUS_DEBUG,
					PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcFirmwareSmBusDebug);
	if (_gProcFirmwareSmBusDebugEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_SMBUS_DEBUG);
	} else {
	...
	_gProcFirmwareSetDQMemValueEntry = proc_create(PROC_NODE_FIRMWARE_SET_DQMEM_VALUE,
					   PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcFirmwareSetDQMemValue);
	if (_gProcFirmwareSetDQMemValueEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_SET_DQMEM_VALUE);
	} else {
...
#ifdef CONFIG_ENABLE_ITO_MP_TEST
	_gProcMpTestEntry = proc_create(PROC_NODE_MP_TEST, PROCFS_AUTHORITY, _gProcDeviceEntry,
					&_gProcMpTest);
	if (_gProcMpTestEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n", PROC_NODE_MP_TEST);
	} else {
	...
	_gProcMpTestLogEntry = proc_create(PROC_NODE_MP_TEST_LOG, PROCFS_AUTHORITY,
					   _gProcDeviceEntry, &_gProcMpTestLog);
	if (_gProcMpTestLogEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n", PROC_NODE_MP_TEST_LOG);
	} else {
		DBG(&g_I2cClient->dev, "Create procfs file node(%s) OK!\n", PROC_NODE_MP_TEST_LOG);
	...
	_gProcMpTestFailChannelEntry = proc_create(PROC_NODE_MP_TEST_FAIL_CHANNEL,
				       PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcMpTestFailChannel);
	if (_gProcMpTestFailChannelEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_MP_TEST_FAIL_CHANNEL);
	} else {
	...
	_gProcMpTestScopeEntry = proc_create(PROC_NODE_MP_TEST_SCOPE, PROCFS_AUTHORITY,
					     _gProcDeviceEntry, &_gProcMpTestScope);
	if (_gProcMpTestScopeEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_MP_TEST_SCOPE);
	} else {
...
#ifdef CONFIG_ENABLE_CHIP_TYPE_MSG28XX
	_gProcMpTestLogALLEntry = proc_create(PROC_NODE_MP_TEST_LOG_ALL, PROCFS_AUTHORITY,
					      _gProcDeviceEntry, &_gProcMpTestLogAll);
	if (_gProcMpTestLogALLEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_MP_TEST_LOG_ALL);
	} else {
	...
	_gProcFirmwareModeEntry = proc_create(PROC_NODE_FIRMWARE_MODE, PROCFS_AUTHORITY,
					      _gProcDeviceEntry, &_gProcFirmwareMode);
	if (_gProcFirmwareModeEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_MODE);
	} else {
	...
	_gProcFirmwareSensorEntry = proc_create(PROC_NODE_FIRMWARE_SENSOR, PROCFS_AUTHORITY,
						_gProcDeviceEntry, &_gProcFirmwareSensor);
	if (_gProcFirmwareSensorEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_SENSOR);
	} else {
	...
	_gProcFirmwarePacketHeaderEntry = proc_create(PROC_NODE_FIRMWARE_PACKET_HEADER,
					  PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcFirmwarePacketHeader);
	if (_gProcFirmwarePacketHeaderEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FIRMWARE_PACKET_HEADER);
	} else {
	...
	_gProcQueryFeatureSupportStatusEntry = proc_create(PROC_NODE_QUERY_FEATURE_SUPPORT_STATUS,
					       PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcQueryFeatureSupportStatus);
	if (_gProcQueryFeatureSupportStatusEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_QUERY_FEATURE_SUPPORT_STATUS);
	} else {
	...
	_gProcChangeFeatureSupportStatusEntry = proc_create(
			PROC_NODE_CHANGE_FEATURE_SUPPORT_STATUS, PROCFS_AUTHORITY, _gProcDeviceEntry,
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
	_gProcGestureWakeupModeEntry = proc_create(PROC_NODE_GESTURE_WAKEUP_MODE,
				       PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcGestureWakeupMode);
	if (_gProcGestureWakeupModeEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_GESTURE_WAKEUP_MODE);
	} else {
...
#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
	_gProcGestureDebugModeEntry = proc_create(PROC_NODE_GESTURE_DEBUG_MODE, PROCFS_AUTHORITY,
				      _gProcDeviceEntry, &_gProcGestureDebugMode);
	if (_gProcGestureDebugModeEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_GESTURE_DEBUG_MODE);
	} else {
...
#ifdef CONFIG_ENABLE_GESTURE_INFORMATION_MODE
	_gProcGestureInforModeEntry = proc_create(PROC_NODE_GESTURE_INFORMATION_MODE,
				      PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcGestureInforMode);
	if (_gProcGestureInforModeEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_GESTURE_INFORMATION_MODE);
	} else {
...
#ifdef CONFIG_ENABLE_COUNT_REPORT_RATE
	_gProcReportRateEntry = proc_create(PROC_NODE_REPORT_RATE, PROCFS_AUTHORITY,
					    _gProcDeviceEntry, &_gProcReportRate);
	if (_gProcReportRateEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n", PROC_NODE_REPORT_RATE);
	} else {
		DBG(&g_I2cClient->dev, "Create procfs file node(%s) OK!\n", PROC_NODE_REPORT_RATE);
	...
	_gProcGloveModeEntry = proc_create(PROC_NODE_GLOVE_MODE, PROCFS_AUTHORITY,
	...
	_gProcOpenGloveModeEntry = proc_create(PROC_NODE_OPEN_GLOVE_MODE, PROCFS_AUTHORITY,
					       _gProcDeviceEntry, &_gProcOpenGloveMode);
	if (_gProcOpenGloveModeEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_OPEN_GLOVE_MODE);
	} else {
	...
	_gProcCloseGloveModeEntry = proc_create(PROC_NODE_CLOSE_GLOVE_MODE, PROCFS_AUTHORITY,
						_gProcDeviceEntry, &_gProcCloseGloveMode);
	if (_gProcCloseGloveModeEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_CLOSE_GLOVE_MODE);
	} else {
	...
	_gProcLeatherSheathModeEntry = proc_create(PROC_NODE_LEATHER_SHEATH_MODE,
				       PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcLeatherSheathMode);
	if (_gProcLeatherSheathModeEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_LEATHER_SHEATH_MODE);
	} else {
...
#ifdef CONFIG_ENABLE_JNI_INTERFACE
	_gProcJniMethodEntry = proc_create(PROC_NODE_JNI_NODE, PROCFS_AUTHORITY,
	...
	_gProcSeLinuxLimitFirmwareUpdateEntry = proc_create(
			PROC_NODE_SELINUX_LIMIT_FIRMWARE_UPDATE, PROCFS_AUTHORITY, _gProcDeviceEntry,
	...
	_gProcForceFirmwareUpdateEntry = proc_create(PROC_NODE_FORCE_FIRMWARE_UPDATE,
					 PROCFS_AUTHORITY, _gProcDeviceEntry, &_gProcForceFirmwareUpdate);
	if (_gProcForceFirmwareUpdateEntry == NULL) {
		DBG(&g_I2cClient->dev, "Failed to create procfs file node(%s)!\n",
		    PROC_NODE_FORCE_FIRMWARE_UPDATE);
	} else {
