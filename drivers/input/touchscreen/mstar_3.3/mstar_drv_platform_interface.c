		...
		if (*pBlank == FB_BLANK_UNBLANK) {
...
#ifdef CONFIG_ENABLE_GESTURE_WAKEUP
#ifdef CONFIG_ENABLE_HOTKNOT
			if (g_HotKnotState != HOTKNOT_BEFORE_TRANS_STATE &&
			    g_HotKnotState != HOTKNOT_TRANS_STATE && g_HotKnotState != HOTKNOT_AFTER_TRANS_STATE)
#endif /* CONFIG_ENABLE_HOTKNOT */
			{
#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
...
#ifdef CONFIG_ENABLE_CHARGER_DETECTION
			{
				...
				if (strnstr(szChargerStatus, "Charging", sizeof(szChargerStatus)) != NULL ||
				    ...
				    strnstr(szChargerStatus, "Fully charged", sizeof(szChargerStatus)) != NULL) { /* Charging */
...

{
	...
	pr_info("%s: Mstar enter\n", __func__);
	if (*ppos) {    /* ADB call again */
		/* pr_info("[HEAD]wr: %d", cmd_head.wr); */
		pr_info("[PARAM]size: %d, *ppos: %d", size, (int)*ppos);
		pr_info("[TOOL_READ]ADB call again, return it.");
		return 0;
	}

	len += snprintf(page + len, size - len, "Manufacturer : %s\n", "MSTAR");
	len += snprintf(page + len, size - len, "chip type : 2XXX\n");
	len += snprintf(page + len, size - len, "SW ID : %d\n", global_eSwId);
	len += snprintf(page + len, size - len, "nMajor : %04x nMinor : %04x\n", global_nMajor,
		       global_nMinor);
	/* len += snprintf(page + len, "pVersion : %d\n", pVersion_N817); */
...
{
	s32 nRetVal = 0;
	struct proc_dir_entry *dir = NULL, *refresh = NULL;

	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);

	if (DrvPlatformLyrTouchDeviceRequestGPIO(pClient) < 0)
		return 0;
	...
	if (dir == NULL)
		pr_err("proc mkdir touchscreen failed!!!\n");

	refresh = proc_create("ts_information", 0664, dir, &proc_ops);
	if (refresh == NULL)
		pr_err("proc create ts_information failed!!!\n");
