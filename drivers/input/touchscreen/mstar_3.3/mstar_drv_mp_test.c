...
static void _DrvMpTestMutualICDebugShowArray(void *pBuf, u16 nLen, int nDataType,
		int nCarry, int nChangeLine);
...
static u16 _DrvMpTestItoTestMsg21xxaGetDataOut(s16 *pRawData)
{
	...
	RegSet16BitValue((0x3d << 8) | (MSG21XXA_REG_INTR_FIQ_MASK << 1),
			 (nRegInt & (u16)(~SELF_IC_FIQ_E_FRAME_READY_MASK)));
...
static void _DrvMpTestItoOpenTestMsg21xxaFirst(u8 nItemId, s16 *pRawData, s8 *pDataFlag)
{
	...
	if (nItemId == 40) {
		...
		if (_gSelfICIsEnable2R) {
			...
			nTotalSensor = _gSelfICItoTestTriangleNum / 2 + _gSelfICItoTestKeyNum +
				       _gSelfICItoTestDummyNum;
		}
	} else if (nItemId == 41) {
		...
		nTotalSensor = _gSelfICItoTestTriangleNum + _gSelfICItoTestKeyNum +
			       _gSelfICItoTestDummyNum;
	}
...
static void _DrvMpTestItoShortTestMsg21xxaFirst(u8 nItemId, s16 *pRawData, s8 *pDataFlag)
{
	...
	if ((_gSelfICItoTestTriangleNum + _gSelfICItoTestKeyNum + _gSelfICItoTestDummyNum) % 2 !=
	    0) {
		nNumOfSensorMapping1 = (_gSelfICItoTestTriangleNum + _gSelfICItoTestKeyNum +
					_gSelfICItoTestDummyNum) / 2 + 1;
		nNumOfSensorMapping2 = nNumOfSensorMapping1;
	} else {
		nNumOfSensorMapping1 = (_gSelfICItoTestTriangleNum + _gSelfICItoTestKeyNum +
					_gSelfICItoTestDummyNum) / 2;
		nNumOfSensorMapping2 = nNumOfSensorMapping1;
...
static void _DrvMpTestItoTestMsg22xxSendDataIn(u8 nStep, u16 nRiuWriteLength)
{
	...
	RegSet16BitValue(0x1180,
			 MSG22XX_RIU_BASE_ADDR); /* sensor mux sram read/write base address */
...
static void _DrvMpTestItoTestMsg22xxSetC(u8 nCSubStep)
{
	...
	nCSubNew = (nCSubStep > MSG22XX_CSUB_REF_MAX) ? MSG22XX_CSUB_REF_MAX :
		   nCSubStep; /* 6 bits */
	nCSubNew = (nCSubNew | (nCSubNew << 8) | (nCSubNew << 16) | (nCSubNew << 24));

	nRegVal = RegGet16BitValue(
			  0x11C8); /* csub sel overwrite enable, will referance value of 11C0 */
	...
	if (nRegVal == 0x000F) {
		...
		RegSet16BitValueOn(0x1192,
				   BIT4);   /* force on enable sensor mux  and csub sel sram clock */
		RegSet16BitValueOff(0x1192, BIT5);   /* mem clk sel */
		RegSet16BitValueOff(0x1100, BIT3);   /* tgen soft rst */
...
static void _DrvMpTestItoTestMsg22xxRegisterReset(void)
{
	...
	RegSet16BitValueOff(0x1240, 0xFFFF);
	RegSet16BitValueOff(0x1242, 0x00FF);

	RegSet16BitValueOn(0x1212, 0xFFFF);
	RegSet16BitValueOn(0x1214, 0x00FF);

	RegSet16BitValueOn(0x121E, 0xFFFF);
	RegSet16BitValueOn(0x1220, 0x00FF);

	RegSet16BitValueOff(0x120E, 0xFFFF);
	RegSet16BitValueOff(0x1210, 0x00FF);
...
static void _DrvMpTestItoTestMsg22xxRegisterResetPatch(void)
{
	DBG(&g_I2cClient->dev, "*** %s() *** _gIsOldFirmwareVersion = %d\n", __func__,
	    _gIsOldFirmwareVersion);

	if (_gIsOldFirmwareVersion ==
	    0) { /* Only need to patch register reset for MSG22XX firmware older than V01.005.02; */
		return;
	}
	...
	RegSet16BitValueByAddressMode(0x115E, 0x0000,
				      ADDRESS_MODE_16BIT); /* sensor and gr ov enable */

	RegSet16BitValueByAddressMode(0x124E, 0x000F, ADDRESS_MODE_16BIT); /* bypass mode */
}

static void _DrvMpTestItoTestMsg22xxGetChargeDumpTime(u16 nMode, u16 *pChargeTime,
		u16 *pDumpTime)
{
	u16 nChargeTime = 0, nDumpTime = 0;
	u16 nMinChargeTime = 0xFFFF, nMinDumpTime = 0xFFFF, nMaxChargeTime = 0x0000,
	    nMaxDumpTime = 0x0000;
...
static void _DrvMpTestItoOpenTestMsg22xxFirst(u8 nItemId, s16 *pRawData, s8 *pDataFlag)
{
	...
	RegSet16BitValue(0x1216, (nSubFrameNum - 1) <<
			 1); /* subframe numbers, 0:1subframe, 1:2subframe */
	...
	if (nItemId == 40) {
		...
		RegSet16BitValue(0x11C8, 0x000F);
		RegSet16BitValue(0x1174, 0x0F06);
		RegSet16BitValue(0x1208, 0x0006); /* PRS1 */
		RegSet16BitValue(0x1240, 0xFFFF);
		RegSet16BitValue(0x1242, 0x00FF);
...
static void _DrvMpTestItoShortTestMsg22xxFirst(u8 nItemId, s16 *pRawData, s8 *pDataFlag)
{
	...
	if ((nItemId == 0 && _gSelfICIsEnable2R == 1) || (nItemId == 1 &&
	...
	if ((nItemId == 0 && _gSelfICIsEnable2R == 1) || (nItemId == 1 &&
		...
		/* RegSet16BitValue(0x1110, 0x0060); // 2.4V -> 1.2V */
		RegSet16BitValue(0x11C8,
				 0x000F); /* csub sel overwrite enable, will referance value of 11C0 */
		RegSet16BitValue(0x1174,
				 0x0000); /* [11:8] 000 : sel active driver for sensor pad that connected to AFE */
...
static u8 _DrvMpTestMsg22xxCheckFirmwareVersion(
	void) /* Only MSG22XX support platform firmware version */
{
	u32 i;
	s32 nDiff;
 ...
 /* {'V', '0', '1', '.', '0', '0', '5', '.', '0', '2'} */
	u8 szCompareFwVersion[10] = {0x56, 0x30, 0x31, 0x2E, 0x30, 0x30, 0x35, 0x2E, 0x30, 0x32};
...
/* Set start address for platform firmware version on info block(Actually, start reading from 0xC1F0) */
	...
	for (i = 0; i < 3; i++) {
		...
		DBG(&g_I2cClient->dev, "szDbBusRxData[%d] = 0x%x , %c\n", i * 4 + 0,
		    szDbBusRxData[i * 4 + 0], szDbBusRxData[i * 4 + 0]); /* add for debug */
		DBG(&g_I2cClient->dev, "szDbBusRxData[%d] = 0x%x , %c\n", i * 4 + 1,
		    szDbBusRxData[i * 4 + 1], szDbBusRxData[i * 4 + 1]); /* add for debug */

		szDbBusRxData[i * 4 + 2] = (nRegData2 & 0xFF);
		szDbBusRxData[i * 4 + 3] = ((nRegData2 >> 8) & 0xFF);

		DBG(&g_I2cClient->dev, "szDbBusRxData[%d] = 0x%x , %c\n", i * 4 + 2,
		    szDbBusRxData[i * 4 + 2], szDbBusRxData[i * 4 + 2]); /* add for debug */
		DBG(&g_I2cClient->dev, "szDbBusRxData[%d] = 0x%x , %c\n", i * 4 + 3,
		    szDbBusRxData[i * 4 + 3], szDbBusRxData[i * 4 + 3]); /* add for debug */
	}
...
static u16 _DrvMpTestItoTestSelfICGetTpType(void)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG21XXA) {
		...
		RegSet16BitValue(0x1600,
				 0xBFF4); /* Set start address for customer firmware version on main block */
...
#endif /* CONFIG_ENABLE_CHIP_TYPE_MSG22XX */
	} else if (nTpType == TP_TYPE_Y) { /* Modify. */
#ifdef CONFIG_ENABLE_CHIP_TYPE_MSG21XXA
		if (g_ChipType == CHIP_TYPE_MSG21XXA) {
...
static ItoTestResult_e _DrvMpTestItoOpenTestSelfICSecond(u8 nItemId)
{
	...
	nTmpJgAvgThMax1 = (nTmpRawDataJg1 / ((_gSelfICItoTestTriangleNum / 2) - 2)) *
			  (100 + SELF_IC_OPEN_TEST_NON_BORDER_AREA_THRESHOLD) / 100;
	nTmpJgAvgThMin1 = (nTmpRawDataJg1 / ((_gSelfICItoTestTriangleNum / 2) - 2)) *
			  (100 - SELF_IC_OPEN_TEST_NON_BORDER_AREA_THRESHOLD) / 100;
	nTmpJgAvgThMax2 = (nTmpRawDataJg2 / 2) * (100 + SELF_IC_OPEN_TEST_BORDER_AREA_THRESHOLD) /
			  100;
	nTmpJgAvgThMin2 = (nTmpRawDataJg2 / 2) * (100 - SELF_IC_OPEN_TEST_BORDER_AREA_THRESHOLD) /
			  100;

	if (nItemId == 40) {
		for (i = 0; i < (_gSelfICItoTestTriangleNum / 2) - 2; i++) {
			if (_gSelfICRawData1[_gSelfIC_MAP40_1[i]] > nTmpJgAvgThMax1 ||
			    _gSelfICRawData1[_gSelfIC_MAP40_1[i]] < nTmpJgAvgThMin1) {
		...
		for (i = 0; i < 2; i++) {
			if (_gSelfICRawData1[_gSelfIC_MAP40_2[i]] > nTmpJgAvgThMax2 ||
			    _gSelfICRawData1[_gSelfIC_MAP40_2[i]] < nTmpJgAvgThMin2) {
		...
		for (i = 0; i < (_gSelfICItoTestTriangleNum / 2) - 2; i++) {
			if (_gSelfICRawData2[_gSelfIC_MAP41_1[i]] > nTmpJgAvgThMax1 ||
			    _gSelfICRawData2[_gSelfIC_MAP41_1[i]] < nTmpJgAvgThMin1) {
		...
		for (i = 0; i < 2; i++) {
			if (_gSelfICRawData2[_gSelfIC_MAP41_2[i]] > nTmpJgAvgThMax2 ||
			    _gSelfICRawData2[_gSelfIC_MAP41_2[i]] < nTmpJgAvgThMin2) {
...
static ItoTestResult_e _DrvMpTestItoOpenTestSelfICSecond2r(u8 nItemId)
{
	...
	nTmpJgAvgThMax1 = (nTmpRawDataJg1 / ((_gSelfICItoTestTriangleNum / 4) - 2)) *
			  (100 + SELF_IC_OPEN_TEST_NON_BORDER_AREA_THRESHOLD) / 100;
	nTmpJgAvgThMin1 = (nTmpRawDataJg1 / ((_gSelfICItoTestTriangleNum / 4) - 2)) *
			  (100 - SELF_IC_OPEN_TEST_NON_BORDER_AREA_THRESHOLD) / 100;
	nTmpJgAvgThMax2 = (nTmpRawDataJg2 / 2) * (100 + SELF_IC_OPEN_TEST_BORDER_AREA_THRESHOLD) /
			  100;
	nTmpJgAvgThMin2 = (nTmpRawDataJg2 / 2) * (100 - SELF_IC_OPEN_TEST_BORDER_AREA_THRESHOLD) /
			  100;
	nTmpJgAvgThMax3 = (nTmpRawDataJg3 / ((_gSelfICItoTestTriangleNum / 4) - 2)) *
			  (100 + SELF_IC_OPEN_TEST_NON_BORDER_AREA_THRESHOLD) / 100;
	nTmpJgAvgThMin3 = (nTmpRawDataJg3 / ((_gSelfICItoTestTriangleNum / 4) - 2)) *
			  (100 - SELF_IC_OPEN_TEST_NON_BORDER_AREA_THRESHOLD) / 100;
	nTmpJgAvgThMax4 = (nTmpRawDataJg4 / 2) * (100 + SELF_IC_OPEN_TEST_BORDER_AREA_THRESHOLD) /
			  100;
	nTmpJgAvgThMin4 = (nTmpRawDataJg4 / 2) * (100 - SELF_IC_OPEN_TEST_BORDER_AREA_THRESHOLD) /
			  100;
	...
	if (nItemId == 40) {
		...
		for (i = 0; i < (_gSelfICItoTestTriangleNum / 4) - 2; i++) {
			if (_gSelfICRawData1[_gSelfIC_MAP40_3[i]] > nTmpJgAvgThMax3 ||
			    _gSelfICRawData1[_gSelfIC_MAP40_3[i]] < nTmpJgAvgThMin3) {
		...
		for (i = 0; i < 2; i++) {
			if (_gSelfICRawData1[_gSelfIC_MAP40_4[i]] > nTmpJgAvgThMax4 ||
			    _gSelfICRawData1[_gSelfIC_MAP40_4[i]] < nTmpJgAvgThMin4) {
		...
		for (i = 0; i < (_gSelfICItoTestTriangleNum / 4) - 2; i++) {
			if (_gSelfICRawData2[_gSelfIC_MAP41_3[i]] > nTmpJgAvgThMax3 ||
			    _gSelfICRawData2[_gSelfIC_MAP41_3[i]] < nTmpJgAvgThMin3) {
		...
		for (i = 0; i < 2; i++) {
			if (_gSelfICRawData2[_gSelfIC_MAP41_4[i]] > nTmpJgAvgThMax4 ||
			    _gSelfICRawData2[_gSelfIC_MAP41_4[i]] < nTmpJgAvgThMin4) {
...
ITO_TEST_END:
	...
	} else if ((nRetVal1 == ITO_TEST_OK) && ((nRetVal2 != ITO_TEST_OK) ||
...
ITO_TEST_END:
	...
	if ((nRetVal1 != ITO_TEST_OK) && (nRetVal2 == ITO_TEST_OK) && (nRetVal3 == ITO_TEST_OK) &&
		...
		return ITO_TEST_GET_TP_TYPE_ERROR;
			...
			(nRetVal3 != ITO_TEST_OK) || (nRetVal4 != ITO_TEST_OK) || (nRetVal5 != ITO_TEST_OK))) {
		return ITO_TEST_FAIL;
	} else {
		return ITO_TEST_OK;
	}
...
static u16 _DrvMpTestItoTestMsg26xxmAnaGetMutualSubFrameNum(void)
{
	...
	nDriveLineNum = ((nRegData & 0xFF00) >> 8) +
			1; /* Since we only retrieve 8th~12th bit of reg_m_sf_num, 0xFF00 shall be changed to 0x1F00. */
...
static void _DrvMpTestItoTestMsg26xxmGetMutualOneShotRawIir(u16
		wszResultData[][MUTUAL_IC_MAX_CHANNEL_DRV], u16 nDriveLineNum, u16 nSenseLineNum)
{
	u16 nRegData;
	u16 i, j;
	u16 nTemp;
	u16 nReadSize;
	u8 szDbBusTxData[3];
	u8 szShotData1[MSG26XXM_FILTER1_MUTUAL_DELTA_C_NUMBER];
	u8 szShotData2[MSG26XXM_FILTER2_MUTUAL_DELTA_C_NUMBER];
	...
	IicWriteData(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0], 3);
	IicReadData(SLAVE_I2C_ID_DBBUS, &szShotData1[0],
		    MSG26XXM_FILTER1_MUTUAL_DELTA_C_NUMBER); /* 190 */

#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
	msleep(100);
	nReadSize = IicSegmentReadDataByDbBus(0x20, 0x00, &szShotData2[0],
	MSG26XXM_FILTER2_MUTUAL_DELTA_C_NUMBER, MAX_I2C_TRANSACTION_LENGTH_LIMIT); /* 594 */
	...
	szDbBusTxData[2] = 0x00;
	IicWriteData(SLAVE_I2C_ID_DBBUS, &szDbBusTxData[0], 3);
	IicReadData(SLAVE_I2C_ID_DBBUS, &szShotData2[0],
		    MSG26XXM_FILTER2_MUTUAL_DELTA_C_NUMBER); /* 594 */
#endif
	...
	for (j = 0; j < nDriveLineNum; j++) {
		for (i = 0; i < nSenseLineNum; i++) {
			...
			wszResultData[i][j] = (short)nRegData;
				...
				if ((j == 6) && (i > 10)) {
					nRegData =
	(u16)(szShotData2[((j - 6) * 14 + (i - 11)) * 2] | szShotData2[((j - 6) * 14 + (i - 11)) * 2 + 1] << 8);
					wszResultData[i][j] = (short)nRegData;
				} else {
					nRegData =
	(u16)(szShotData2[6 + ((j - 7) * 14 + i) * 2] | szShotData2[6 + ((j - 7) * 14 + i) * 2 + 1] << 8);
					wszResultData[i][j] = (short)nRegData;
				}
			}
		}
	...
	if (_gMutualICTestSwitchMode != 0) {
		if (_DrvMpTestItoTestMsg26xxmSwitchFwMode(nFMode) < 0) {
			if (nFMode == MSG26XXM_MUTUAL) {
				...
				DBG(&g_I2cClient->dev,
				    "*** Msg26xx MP Test# _DrvMpTestItoTestMsg26xxmSwitchMode failed! ***\n");
				return -EPERM;
			}
		}
	} else {
...
static s32 _DrvMpTestMsg26xxmItoOpenTestEntry(void)
{
	...
	for (i = 0; i < _gMutualICSenseLineNum; i++) {
		...
		for (j = 0; j < _gMutualICDriveLineNum; j++) {
			_gMutualICResult[i * _gMutualICDriveLineNum + j] = (4464 * MSG26XXM_OPEN_CSUB_REF_X -
					_gMutualICDeltaC[i * _gMutualICDriveLineNum + j]);
			DBG(&g_I2cClient->dev, "%d  %d  %d\t", _gMutualICResult[i * _gMutualICDriveLineNum + j],
			    4464 * MSG26XXM_OPEN_CSUB_REF_X, _gMutualICDeltaC[i * _gMutualICDriveLineNum + j]);
		}
	}
	...
	for (j = 0; j < (_gMutualICDriveLineNum - 1); j++) {
		for (i = 0; i < _gMutualICSenseLineNum; i++) {
			if (_gMutualICResult[i * _gMutualICDriveLineNum + j] < MSG26XXM_FIR_THRESHOLD) {
				...
				_gTestFailChannelCount++;
				nRetVal = -1;
				DBG(&g_I2cClient->dev, "\nSense%d, Drive%d, MIN_Threshold = %d\t", i, j,
				    _gMutualICResult[i * _gMutualICDriveLineNum + j]);
			}

			if (i > 0) {
				nDelta = _gMutualICResult[i * _gMutualICDriveLineNum + j] > nPrev
					? (_gMutualICResult[i * _gMutualICDriveLineNum + j] - nPrev)
					: (nPrev - _gMutualICResult[i * _gMutualICDriveLineNum + j]);
				if (nDelta > nPrev * MUTUAL_IC_FIR_RATIO / 100) {
					if (_gMutualICTestFailChannel[i * _gMutualICDriveLineNum + j] == 0) {
						_gMutualICTestFailChannel[i * _gMutualICDriveLineNum + j] = 1;
						_gTestFailChannelCount++;
					}
				nRetVal = -1;
				DBG(&g_I2cClient->dev, "\nSense%d, Drive%d, MRatio%d,%d\t", i, j, nDelta, nPrev);
				}
			}
			nPrev = _gMutualICResult[i * _gMutualICDriveLineNum + j];
		}
...
static void _DrvMpTestItoShortTestMsg26xxmSetPAD2GPO(u8 nItemID)
{
	...
	/* /cts sw overwrite */
	{
		_DrvMpTestItoTestMsg26xxmSendDataIn(0x1E66, gpioNum,
						    &gpioSetting[0]);   /* /who -> will be controlled */
		_DrvMpTestItoTestMsg26xxmSendDataIn(0x1E6C, gpioNum,
						    &gpioEnabling[0]);   /* /who -> enable sw overwrite */
		_DrvMpTestItoTestMsg26xxmSendDataIn(0x1E72, gpioNum,
						    &gpioZero[0]);       /* /who -> set2GPO */
		_DrvMpTestItoTestMsg26xxmSendDataIn(0x1E78, gpioNum,
						    &gpioZero[0]);       /* /who -> GPO2GND */
	}

	for (j = 0; j < gpioNum; j++) {
		if (gpioPIN[j] == MSG26XXM_PIN_GUARD_RING) {
			u16 u16RegData;
...
static void _DrvMpTestMsg26xxmItoShortTest(u8 nItemID)
{
	...
	for (i = 0; i < MUTUAL_IC_MAX_MUTUAL_NUM; i++) {
		if ((_gMutualICDeltaC[i] <= -(MUTUAL_IC_IIR_MAX)) ||
		    (_gMutualICTempDeltaC[i] <= -(MUTUAL_IC_IIR_MAX)) ||
		    (_gMutualICDeltaC[i] >= (MUTUAL_IC_IIR_MAX)) ||
		    (_gMutualICTempDeltaC[i] >= (MUTUAL_IC_IIR_MAX))) {
...
static ItoTestResult_e _DrvMpTestItoShortTestMsg26xxmJudge(u8 nItemID)
{
	...
	for (i = 0; i < nTestPinLength; i++) {
		...
		if (_DrvMpTestMutualICCheckValueInRange(_gMutualICDeltaC[i],
				MSG26XXM_SHORT_VALUE,
				-MSG26XXM_SHORT_VALUE) == 0) {
			nRetVal = ITO_TEST_FAIL;
			_gTestFailChannelCount++;
			DBG(&g_I2cClient->dev,
				"_gMsg26xxmShortTestChannel i = %d, _gMutualICDeltaC = %d\t", i,
				_gMutualICDeltaC[i]);
		}
	}
...
static ItoTestResult_e _DrvMpTestMsg26xxmItoShortTestEntry(void)
{
	...
	DBG(&g_I2cClient->dev,
	    "nRetVal1 = %d, nRetVal2 = %d, nRetVal3 = %d, nRetVal4 = %d, nRetVal5 = %d\n", nRetVal1,
	    nRetVal2, nRetVal3, nRetVal4, nRetVal5);
...
static s32 _DrvMpTestItoTestMsg26xxmGetWaterProofDeltaC(s32 *pTarget)
{
	...
	if (_DrvMpTestItoTestMsg26xxmGetWaterProofOneShotRawIir(wszRawData) < 0) {
		DBG(&g_I2cClient->dev,
		    "*** Msg26xxm WaterProof Test# GetMutualOneShotRawIIR failed! ***\n");
		return -EPERM;
	}
...
s32 _DrvMpTestMsg26xxmItoWaterProofTestEntry(void)
{
	...
	if (_DrvMpTestItoTestMsg26xxmSwitchMode(_gMutualICTestSwitchMode,
						MSG26XXM_WATERPROOF) < 0) {
	...
	if (nRegData == 0x8bbd) { /* // if polling 0X8BBD, means the FW is NOT supporting WP. */
		DBG(&g_I2cClient->dev,
		    "*** Msg26xxm WaterProof Test# No supporting this function! ***\n");
		return -EPERM;
	}
	...
	for (i = 0; i < 12; i++) {
		if (aResultTemp[i] <
		    WATERPROOFVALUE) {  /* change comparison way because float computing in driver is prohibited */
			_gMutualICTestFailChannel[i] = 1;
			_gTestFailChannelCount++;
			nRetVal = -1;
		} else {
...
static s32 _DrvMpTestItoTestMsg28xxTriggerMutualOneShot(s16 *pResultData, u16 *pSenNum,
		u16 *pDrvNum)
{
	...
	if (nDriMode == 2) { /* for short test */
			...
			if (_gMsg28xx_PATTERN_TYPE == 5) {
				for (j = 0; j < nAfeOpening; j++) {
					...
					pResultData[i * nAfeOpening + j]
						= (s16)(aShotData[2 * j] | aShotData[2 * j + 1] << 8);

					if (nDataShift == 0 && (j == nAfeOpening - 1)) {
					pResultData[i * nAfeOpening + j]
						= (s16)(aShotData[2 * (j + 1)] | aShotData[2 * (j + 1) + 1] << 8);
					}
				}
			} else {
				for (j = 0; j < nAfeOpening; j++) {
					pResultData[i * MUTUAL_IC_MAX_CHANNEL_DRV + j]
						= (s16)(aShotData[2 * j] | aShotData[2 * j + 1] << 8);
		...
		*pDrvNum = nAfeOpening;
		...
		for (i = 0; i < nSF; i++) {
			memset(aShotData, 0, sizeof(aShotData));
			_DrvMpTestItoTestDBBusReadDQMemStart();
			RegGetXBitValue(nAddr + i * nAddrNextSF, aShotData, 392,
			...
			for (j = 0; j < nMaxDataNumOfOneSF; j++) {
				...
				pShotDataAll[i * nMaxDataNumOfOneSF + j]
					= (s16)(aShotData[2 * j] | aShotData[2 * j + 1] << 8);
				DBG(&g_I2cClient->dev,
				"_DrvMpTestItoTestMsg28xxTriggerMutualOneShot_1 : %02X, %02X, %04X\n",
				aShotData[2 * j + 1], aShotData[2 * j],
				pShotDataAll[i * nMaxDataNumOfOneSF + j]);
				...
				if (nDataShift == 0 && j == (nMaxDataNumOfOneSF - 1)) {
					pShotDataAll[i * nMaxDataNumOfOneSF + j]
						= (s16)(aShotData[2 * (j + 1)] | aShotData[2 * (j + 1) + 1] << 8);
					DBG(&g_I2cClient->dev,
					"_DrvMpTestItoTestMsg28xxTriggerMutualOneShot_2 : %02X, %02X, %04X\n",
					aShotData[2 * (j + 1) + 1], aShotData[2 * (j + 1)],
					pShotDataAll[i * nMaxDataNumOfOneSF + j]);
				}
			}
			DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# aShotData ***\n");
			_DrvMpTestMutualICDebugShowArray(aShotData,
							 _gMutualICSenseLineNum * _gMutualICDriveLineNum,
							 8, 16, _gMutualICDriveLineNum);
		}

		DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# pShotDataAll ***\n");
		_DrvMpTestMutualICDebugShowArray(pShotDataAll,
						 _gMutualICSenseLineNum * _gMutualICDriveLineNum,
						 -16, 16, _gMutualICDriveLineNum);

		DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# pShotDataAll ***\n");
		_DrvMpTestMutualICDebugShowArray(pShotDataAll,
						 _gMutualICSenseLineNum * _gMutualICDriveLineNum,
						 -16, 10, _gMutualICDriveLineNum);
		...
		for (k = 0; k < nSF; k++) {
			for (i = k * nAfeOpening; i < nAfeOpening * (k + 1); i++) { /* Sen */
				...
				pResultData[i * MUTUAL_IC_MAX_CHANNEL_DRV + j] =
	pShotDataAll[k * nMaxDataNumOfOneSF + (j + (i - nAfeOpening * k) * nDriOpening)];
				}
			}
		}
...
static s32 _DrvMpTestItoTestMsg28xxGetMutualOneShotRawIIR(s16 *nResultData, u16 *pSenNum,
		u16 *pDrvNum)
{
...
static s32 _DrvMpTestItoTestMsg28xxGetDeltaC(s32 *pDeltaC)
{
	...
	pRawData =
		kzalloc(sizeof(s16) * MUTUAL_IC_MAX_CHANNEL_SEN * 2 * MUTUAL_IC_MAX_CHANNEL_DRV,
			   GFP_KERNEL);

	if (_DrvMpTestItoTestMsg28xxGetMutualOneShotRawIIR(pRawData,
								&nSenNumBak,
								&nDrvNumBak) < 0) {
	...
	_DrvMpTestMutualICDebugShowArray(pRawData,
					 _gMutualICSenseLineNum * _gMutualICDriveLineNum,
					 -16, 10, _gMutualICSenseLineNum);
	...
	_DrvMpTestMutualICDebugShowArray(aRawDataOverlapDone,
					 _gMutualICSenseLineNum * _gMutualICDriveLineNum,
					 -16, 10, _gMutualICSenseLineNum);
	...
	_DrvMpTestMutualICDebugShowArray(pDeltaC,
					_gMutualICSenseLineNum * _gMutualICDriveLineNum,
					 -32, 10, _gMutualICSenseLineNum);
...
static void _DrvMpTestItoTestMsg28xxAnaSwReset(void)
{
	...
	/* / reset ANA */
	RegSet16BitValueOn(0x1002, (BIT0 | BIT1 | BIT2 |
				    BIT3));     /* /reg_tgen_soft_rst: 1 to reset */
...
static void _DrvMpTestItoOpenTestMsg28xxOnCellUpdateAnaChargeDumpSetting(void)
{
	...
	RegSet16BitValueByAddressMode(0x101a, 0x0028,
				      ADDRESS_MODE_16BIT);  /* post idle time in sub0 */
	RegSet16BitValueByAddressMode(0x101b, 0x0028,
				      ADDRESS_MODE_16BIT);  /* post idle time in sub1 */
	RegSet16BitValueByAddressMode(0x101c, 0x0028,
				      ADDRESS_MODE_16BIT);  /* post idle time in sub2 */
	RegSet16BitValueByAddressMode(0x101d, 0x0028,
				      ADDRESS_MODE_16BIT);  /* post idle time in sub3 */
	RegSet16BitValueByAddressMode(0x101e, 0x0028,
				      ADDRESS_MODE_16BIT);  /* post idle time in sub4 */
...
void _DrvMpTestItoTestMsg28xxRegGet16BitByteValueBuf(u16 nAddr, u8 *pBuf, u16 nLen)
{
	...
	u8 *pRxData = kzalloc(nLen, sizeof(u8));
...
void _DrvMpTestItoTestMsg28xxWriteDQMem_8bit(u16 addr, u8 data)
{
	...
	_DrvMpTestItoTestMsg28xxRegGet16BitByteValueBuf(read_addr, aReadBuf,
			ARRAY_SIZE(aReadBuf);

	aReadBuf[addr % 4] = data;
	DBG(&g_I2cClient->dev, "read_buf:0x%02x 0x%02x 0x%02x  0x%02x", aReadBuf[0], aReadBuf[1],
	    aReadBuf[2], aReadBuf[3]);
...
s32 _DrvMpTestItoOpenTestMsg28xxOnCellOpenVaValue(void)
{
	...
	for (nIsf = 0; nIsf < nSF; nIsf++) {
		memset(aShotData, 0, ARRAY_SIZE(aShotData));
		_DrvMpTestItoTestDBBusReadDQMemStart();
		/* RegGetXByteValue(u8ShotData, addr + 0x20 * isf, sizeof(u8ShotData) / sizeof(u8ShotData[0])); */
		RegGetXBitValue(nAddr + 0x20 * nIsf, aShotData, ARRAY_SIZE(aShotData)),
				MAX_I2C_TRANSACTION_LENGTH_LIMIT);
		DBG(&g_I2cClient->dev, "*** %s() _ Before***\n", __func__);
		_DrvMpTestMutualICDebugShowArray(aShotData, ARRAY_SIZE(aShotData), 8,
						 16, 10);
...
static s32 _DrvMpTestItoOpenTestMsg28xxOpenJudge(u16 nItemID, s8 pNormalTestResult[][2],
		u16 pNormalTestResultCheck[][13]/*, u16 nDriOpening*/)
{
	s32 nRetVal = 0;
	...
	for (i = 0; i < _gMutualICSenseLineNum * _gMutualICDriveLineNum; i++) {
		...
		if ((_gMsg28xx_MUTUAL_KEY == 1 || _gMsg28xx_MUTUAL_KEY == 2) &&
		    (_gMsg28xx_KEY_NUM != 0)) {
			if ((_gMutualICSenseLineNum < _gMutualICDriveLineNum) &&
			    ((i + 1) % _gMutualICDriveLineNum == 0)) {
			...
			if ((_gMutualICSenseLineNum > _gMutualICDriveLineNum) &&
			    (i > (_gMutualICSenseLineNum - 1) * _gMutualICDriveLineNum - 1)) {
				_gMutualICResult[i] = -32000;
				for (k = 0; k < _gMsg28xx_KEY_NUM; k++) {
			if (((i + 1) - (_gMutualICSenseLineNum - 1) * _gMutualICDriveLineNum)
				== _gMsg28xx_KEYSEN[k]) {
				_gMutualICResult[i] = 1673 * nCSub - _gMutualICDeltaC[i];
					}
				}
	...
	for (j = 0; j < nRowNum; j++) {
		...
		for (i = 0; i < nColumnNum; i++) {
			if (_DrvMpTestMutualICCheckValueInRange(_gMutualICResult[i * _gMutualICDriveLineNum + j],
				(s32)(nAvg + nAvg * MSG28XX_DC_RANGE / 100),
				(s32)(nAvg - nAvg * MSG28XX_DC_RANGE / 100)) == 0) {
				_gMutualICTestFailChannel[i * _gMutualICDriveLineNum + j] = 1;
				_gTestFailChannelCount++;
				nRetVal = -1;
			}
...
static s32 _DrvMpTestItoOpenTestMsg28xxOnCellOpenJudge(u16 nItemID,
		s8 pNormalTestResult[], u16 pNormalTestResultCheck[]/*, u16 nDriOpening*/)
{
	s32 nRetVal = 0;
	u16 nCSub = _gMsg28xx_CSUB_REF;
	u16 nRowNum = 0, nColumnNum = 0;
	...
	s32 ratioAvgBorder1000 = 0, ratioAvgBorderMax1000 = 0, ratioAvgBorderMin1000 = 0,
	    passCount1 = 0;
	...
	memset(_gMutualICOnCellOpenTestRatioBorder1000, 0,
	       sizeof(_gMutualICOnCellOpenTestRatioBorder1000));
	memset(_gMutualICOnCellOpenTestRatioMove1000, 0,
	       sizeof(_gMutualICOnCellOpenTestRatioMove1000));
	memset(_gMutualICOnCellOpenTestRatioBorderMove1000, 0,
	       sizeof(_gMutualICOnCellOpenTestRatioBorderMove1000));
	...
	for (i = 0; i < _gMutualICSenseLineNum * _gMutualICDriveLineNum; i++) {
		...
		if (_gMutualICDeltaC[i] != MSG28XX_UN_USE_SENSOR) {
			if ((_gMsg28xx_PATTERN_TYPE == 5) && (_gMsg28xx_PATTERN_MODEL == 1)) {
				_gMutualICResult[i] =
					bg_per_csub * nCSub - _gMutualICDeltaC[i];
			} else {
				_gMutualICResult[i] =
					1673 * nCSub - _gMutualICDeltaC[i];
			}
		} else {
	...
	DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# Show _gMutualICResult ***\n");
	_DrvMpTestMutualICDebugShowArray(_gMutualICResult, nRowNum * nColumnNum, -32, 10,
					 nColumnNum);
	memcpy(_gMutualICOnCellOpenTestResultData, _gMutualICResult,
	       sizeof(_gMutualICOnCellOpenTestResultData));
	DBG(&g_I2cClient->dev,
	    "*** Msg28xx Open Test# Show _gMutualICOnCellOpenTestGoldenChannel ***\n");
	_DrvMpTestMutualICDebugShowArray(_gMutualICOnCellOpenTestGoldenChannel,
					 nRowNum * nColumnNum, -32, 10, nColumnNum);
	DBG(&g_I2cClient->dev,
	    "*** Msg28xx Open Test# Show _gMutualICOnCellOpenTestGoldenChannelMax ***\n");
	_DrvMpTestMutualICDebugShowArray(_gMutualICOnCellOpenTestGoldenChannelMax,
					 nRowNum * nColumnNum, -32, 10, nColumnNum);
	DBG(&g_I2cClient->dev,
	    "*** Msg28xx Open Test# Show _gMutualICOnCellOpenTestGoldenChannelMin ***\n");
	_DrvMpTestMutualICDebugShowArray(_gMutualICOnCellOpenTestGoldenChannelMin,
					 nRowNum * nColumnNum, -32, 10, nColumnNum);

	for (k = 0; k < ARRAY_SIZE(_gMutualICDeltaC); k++) {
		if (_gMutualICOnCellOpenTestGoldenChannel[k] == 0) {
			if (k == 0) {
		...
		if (_gMutualICResult[k] != MSG28XX_NULL_DATA) {
			_gMutualICOnCellOpenTestRatio1000[k] = _gMutualICResult[k] * 1000 /
							       _gMutualICOnCellOpenTestGoldenChannel[k];

			if (_DrvMpTestMutualICCheckValueInRange(_gMutualICResult[k],
					_gMutualICOnCellOpenTestGoldenChannelMax[k],
					_gMutualICOnCellOpenTestGoldenChannelMin[k]) == 0) {
				DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# Golden FAIL: %d ***\n", k);
				DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# _gMutualICResult: %d ***\n",
				    _gMutualICResult[k]);
				DBG(&g_I2cClient->dev,
				    "*** Msg28xx Open Test# _gMutualICOnCellOpenTestGoldenChannelMax: %d ***\n",
				    _gMutualICOnCellOpenTestGoldenChannelMax[k]);
				DBG(&g_I2cClient->dev,
				    "*** Msg28xx Open Test# _gMutualICOnCellOpenTestGoldenChannelMin: %d ***\n",
				    _gMutualICOnCellOpenTestGoldenChannelMin[k]);
				pNormalTestResult[0] = 1;
				pNormalTestResultCheck[k] = (u16)(((k / _gMutualICDriveLineNum) + 1) * 100 +
					((k % _gMutualICDriveLineNum) + 1));
			} else {
				pNormalTestResultCheck[k] = MSG28XX_PIN_NO_ERROR;

				if ((_gMsg28xx_PATTERN_TYPE == 3)
					&& (_gMsg28xx_KEY_NUM == 0)
					&& ((k % _gMutualICDriveLineNum == 0)
					|| ((k + 1) % _gMutualICDriveLineNum == 0))) {
					ratioAvgBorder1000 +=  _gMutualICOnCellOpenTestRatio1000[k];
					passCount1 += 1;
				} else if ((_gMsg28xx_PATTERN_TYPE == 3)
						&& (_gMsg28xx_KEY_NUM != 0)
						&& ((k % _gMutualICDriveLineNum == 0)
						|| ((k + 2) % _gMutualICDriveLineNum == 0))) {
					ratioAvgBorder1000 +=  _gMutualICOnCellOpenTestRatio1000[k];
					passCount1 += 1;
				} else {
	...
	DBG(&g_I2cClient->dev,
	    "*** Msg28xx Open Test# Show  _gMutualICOnCellOpenTestRatio1000 ***\n");
	_DrvMpTestMutualICDebugShowArray(_gMutualICOnCellOpenTestRatio1000, nRowNum * nColumnNum,
					 -32, 10, nColumnNum);
	ratioAvgMax1000 = (s32)(100000 + _gMsg28xx_DC_Ratio_1000 + _gMsg28xx_DC_Ratio_UP_1000) /
			  100;
	...
	DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# ratioAvgMin1000: %d ***\n",
	    ratioAvgMin1000);
	ratioAvgBorderMax1000 = (s32)(100000 + _gMsg28xx_DC_Border_Ratio_1000 +
				      _gMsg28xx_DC_Border_Ratio_UP_1000) / 100;
	...
	if (passCount != 0) {
		if (passCount1 != 0) {
			...
			for (i = 0;
			     i < sizeof(_gMutualICOnCellOpenTestRatio1000) / sizeof(
				     _gMutualICOnCellOpenTestRatio1000[0]); i++) {
				if ((_gMsg28xx_KEY_NUM == 0)
					&& ((i % _gMutualICDriveLineNum == 0)
					|| ((i + 1) % _gMutualICDriveLineNum == 0))) {
					_gMutualICOnCellOpenTestRatioMove1000[i]
						=  _gMutualICOnCellOpenTestRatio1000[i] - ratioAvgBorderMove1000 + 1;
				} else if ((_gMsg28xx_KEY_NUM != 0)
					&& ((i % _gMutualICDriveLineNum == 0)
					|| ((i + 2) % _gMutualICDriveLineNum == 0))) {
					_gMutualICOnCellOpenTestRatioMove1000[i]
						=  _gMutualICOnCellOpenTestRatio1000[i] - ratioAvgBorderMove1000 + 1;
				} else {
					_gMutualICOnCellOpenTestRatioMove1000[i]
						=  _gMutualICOnCellOpenTestRatio1000[i] - ratioAvgMove1000 + 1;
				}
			}
		} else {
			ratioAvgMove1000 = ratioAvg1000 / passCount;
			...
			for (i = 0;
			     i < sizeof(_gMutualICOnCellOpenTestRatio1000) / sizeof(
				     _gMutualICOnCellOpenTestRatio1000[0]); i++) {
				_gMutualICOnCellOpenTestRatioMove1000[i]
					=  _gMutualICOnCellOpenTestRatio1000[i] - ratioAvgMove1000 + 1000;
			}
			DBG(&g_I2cClient->dev,
			    "*** Msg28xx Open Test# Show _gMutualICOnCellOpenTestRatioMove1000 ***\n");
			_DrvMpTestMutualICDebugShowArray(_gMutualICOnCellOpenTestRatioMove1000,
							 nRowNum * nColumnNum, -32, 10, nColumnNum);
			memcpy(_gMutualICOnCellOpenTestResultRatioData,
				_gMutualICOnCellOpenTestRatioMove1000,
			       sizeof(_gMutualICOnCellOpenTestResultRatioData));
		}
	} else {
		memcpy(_gMutualICOnCellOpenTestRatio1000,
			_gMutualICOnCellOpenTestRatioMove1000,
		       sizeof(_gMutualICOnCellOpenTestRatio1000));
	}

	for (j = 0; j < ARRAY_SIZE(_gMutualICDeltaC); j++) {
		if (_gMutualICOnCellOpenTestGoldenChannel[j] == 0) {
			if (j == 0) {
		...
		if (pNormalTestResultCheck[j] == MSG28XX_PIN_NO_ERROR) {

			if (_gMutualICResult[j] != MSG28XX_NULL_DATA) {
				if ((_gMsg28xx_PATTERN_TYPE == 3)
					&& (_gMsg28xx_KEY_NUM == 0)
					&& ((j % _gMutualICDriveLineNum == 0)
					|| ((j + 1) % _gMutualICDriveLineNum == 0))) {
	if (_DrvMpTestMutualICCheckValueInRange(_gMutualICOnCellOpenTestRatioMove1000[j],
					ratioAvgBorderMax1000, ratioAvgBorderMin1000) == 0) {
						DBG(&g_I2cClient->dev, "Msg28xx Open Test# FAIL: %d\n", j);
						pNormalTestResult[1] = 1;
						pNormalTestResultCheck[j]
	= (u16)(((j / _gMutualICDriveLineNum) + 1) * 100 + ((j % _gMutualICDriveLineNum) + 1));
					} else {
					...
					&& (_gMsg28xx_KEY_NUM != 0)
					&& ((j % _gMutualICDriveLineNum == 0)
					|| ((j + 2) % _gMutualICDriveLineNum == 0))) {
	if (_DrvMpTestMutualICCheckValueInRange(_gMutualICOnCellOpenTestRatioMove1000[j],
							ratioAvgBorderMax1000,
							ratioAvgBorderMin1000) == 0) {
						pNormalTestResult[1] = 1;
						pNormalTestResultCheck[j]
	= (u16)(((j / _gMutualICDriveLineNum) + 1) * 100 + ((j % _gMutualICDriveLineNum) + 1));
					} else {
	...
	if (_DrvMpTestMutualICCheckValueInRange(_gMutualICOnCellOpenTestRatioMove1000[j],
							ratioAvgMax1000, ratioAvgMin1000) == 0) {
	...
	DBG(&g_I2cClient->dev,
		"*** Msg28xx Open Test# pNormalTestResult[0]: %d ***\n",
		pNormalTestResult[0]);
	_gMutualICOnCellOpenTestResult[0] = pNormalTestResult[0];
	DBG(&g_I2cClient->dev,
		"*** Msg28xx Open Test# pNormalTestResult[1]: %d ***\n",
		pNormalTestResult[1]);
...
static s32 _DrvMpTestMsg28xxItoTestSwitchFwMode(u16 *pFMode)
{
	...
	if (_gMsg28xx_PATTERN_TYPE == 5) {
		if (_gMsg28xxDeepStandBy == 1) {
			...
			msleep(_gMsg28xxDeepStandByTimeOut);
	...
	if (*pFMode == MUTUAL_SINE) {
		...
		if (abs(nFreqData - _gMsg28xx_FIXED_CARRIER) >= 2) {
			DBG(&g_I2cClient->dev,
			    ...
			    nFreqData, _gMsg28xx_FIXED_CARRIER);
			return -EPERM;
		}
	}
...
static void _DrvMpTestItoOpenTestMsg28xxOnCellCalGoldenRange(void)
{
	...
	for (i = 0; i < MUTUAL_IC_MAX_MUTUAL_NUM; i++) {
		...
		value_up = (s32)_gMsg28xx_DC_RANGE_UP * abs(_gMutualICOnCellOpenTestGoldenChannel[i]) /
			   100;
		_gMutualICOnCellOpenTestGoldenChannelMax[i] = _gMutualICOnCellOpenTestGoldenChannel[i] +
				value + value_up;
		_gMutualICOnCellOpenTestGoldenChannelMin[i] = _gMutualICOnCellOpenTestGoldenChannel[i] -
				value;
	}
}
...
static u16 _DrvMpTestMsg28xxItoTestChooseTpType(void)
{
	...
	if (_gMsg28xxTpType == TP_TYPE_X) { /* Modify. */
		...
		if (_gMsg28xx_PATTERN_TYPE == 5) {
			...
			memcpy(_gMutualICOnCellOpenTestGoldenChannel, g_GoldenChannel_X,
			       sizeof(_gMutualICOnCellOpenTestGoldenChannel));

			_DrvMpTestItoOpenTestMsg28xxOnCellCalGoldenRange();
		}
		/* g_Msg28xxMapVaMutual = g_MapVaMutual_X; */
	} else if (_gMsg28xxTpType == TP_TYPE_Y) { /* Modify. */
		...
		if ((_gMsg28xx_PATTERN_TYPE == 5)) {
			...
			memcpy(_gMutualICOnCellOpenTestGoldenChannel, g_GoldenChannel_Y,
			       sizeof(_gMutualICOnCellOpenTestGoldenChannel));
...
static s32 _DrvMpTestItoShortTestMsg28xxGetValueR(s32 *pTarget)
{
	...
	if (_gMsg28xx_PATTERN_TYPE == 5) {
		...
		pRawData = kzalloc(sizeof(s16) * MUTUAL_IC_MAX_CHANNEL_SEN * 2 *
				   MUTUAL_IC_MAX_CHANNEL_DRV, GFP_KERNEL);
	}

	if (_DrvMpTestItoTestMsg28xxGetMutualOneShotRawIIR(pRawData, &nSenNumBak,
			&nDrvNumBak) < 0) {
...
static s16 _DrvMpTestMsg28xxItoTestChooseVAMapping(u16 nScanMode)
{
	...
	if (_gMsg28xxTpType == TP_TYPE_X) {
		switch (nDrvOpening) {
		case 4:
			if (nScanMode == MSG28XX_KEY_SEPARATE) {
...
static u16 _DrvMpTestMsg28xxGetFirmwareOfficialVersionOnFlash(void)
{
	...
	DBG(&g_I2cClient->dev,
	    "SW ID = 0x%x, szTmpData[0] = 0x%x, szTmpData[1] = 0x%x, szTmpData[2] = 0x%x, szTmpData[3] = 0x%x\n",
...
static s32 _DrvMpTestMsg28xxopen_previous_FW_v1007(u16 nFMode)
{
	...
	if ((_gMsg28xx_PATTERN_TYPE == 5) && (_gMsg28xx_PATTERN_MODEL == 1) &&
		...
		if (_DrvMpTestItoOpenTestMsg28xxObtainOpenValueKeys(_gKeyArray) < 0) {
			DBG(&g_I2cClient->dev,
			    "*** _DrvMpTestItoOpenTestMsg28xxObtainOpenValueKeys failed ***\n");
			return -EPERM;
		}
...
_RETRY_OPEN:
	...
	if (_gMsg28xx_PATTERN_TYPE == 5) {
		if (nFwVer == 0x0007) {
			if ((nScanMode != MSG28XX_KEY_SEPARATE) && (nScanMode != MSG28XX_KEY_COMBINE)) {
				nScanMode = MSG28XX_KEY_SEPARATE;
				g_IsDynamicCode = 0;
			}
		...
		DBG(&g_I2cClient->dev,
		    "*** g_IsDynamicCode = %d , _gMsg28xx_AnaGen_Version = %d.%d.%d.%d ***\n",
		    g_IsDynamicCode, _gMsg28xx_AnaGen_Version[0], _gMsg28xx_AnaGen_Version[1],
		    _gMsg28xx_AnaGen_Version[2], _gMsg28xx_AnaGen_Version[3]);
		if (nScanMode == MSG28XX_KEY_SEPARATE) {
	...
	if (_gMsg28xx_PATTERN_TYPE == 5) {
		nRetVal = _DrvMpTestItoOpenTestMsg28xxOnCellOpenJudge(0, aOnCellNormalTestResult,
				aOnCellNormalTestResultCheck/*, nDrvOpening*/);
	} else {
		nRetVal = _DrvMpTestItoOpenTestMsg28xxOpenJudge(0, aNormalTestResult,
				aNormalTestResultCheck/*, nDrvOpening*/);
	}
...
static void _DrvMpTestMsg28xxItoReadSetting(u16 *pPad2Sense, u16 *pPad2Drive,
		u16 *pPad2GR)
{
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);

	memcpy(_gMsg28xxMuxMem_20_3E_1_Settings, _gMsg28xx_SHORT_N1_MUX_MEM_20_3E,
	       sizeof(u16) * 16);
	memcpy(_gMsg28xxMuxMem_20_3E_2_Settings, _gMsg28xx_SHORT_N2_MUX_MEM_20_3E,
	       sizeof(u16) * 16);
	memcpy(_gMsg28xxMuxMem_20_3E_3_Settings, _gMsg28xx_SHORT_S1_MUX_MEM_20_3E,
	       sizeof(u16) * 16);
	memcpy(_gMsg28xxMuxMem_20_3E_4_Settings, _gMsg28xx_SHORT_S2_MUX_MEM_20_3E,
	       sizeof(u16) * 16);

	if (_gMsg28xx_SHORT_TEST_5_TYPE != 0) {
		if (_gMsg28xx_PATTERN_TYPE == 5) {
			memcpy(_gMsg28xxMuxMem_20_3E_5_Settings, _gMsg28xx_SHORT_GR_MUX_MEM_20_3E,
			       sizeof(u16) * 16);
		} else {
			memcpy(_gMsg28xxMuxMem_20_3E_5_Settings, _gMsg28xx_SHORT_X_MUX_MEM_20_3E,
			       sizeof(u16) * 16);
		}
	}
...
static void _DrvMpTestItoShortTestMsg28xxOnCellSetSensorPADState(u16 state)
{
	...
	for (i = 0; i < 4; i++) {
		RegSet16BitValueByAddressMode(0x1510 + i, 0xFFFF,
					      ADDRESS_MODE_16BIT); /* After DAC overwrite, output DC */
	}
}
...
static s32 _DrvMpTestMsg28xxItoShortTest(u8 nItemID)
{
	...
	if (_gMsg28xx_PATTERN_TYPE == 5) {
		...
		/* coefficient */
		RegSet16BitValueByAddressMode(0x136B, 0x10000 / 0x0020,
					      ADDRESS_MODE_16BIT);/* 65536/ sample */
	...
	for (i = 0; i < 65; i++) {
		if (_gMsg28xx_PATTERN_TYPE == 5) {
			if ((abs(_gMutualICDeltaC2[i]) < MSG28XX_IIR_MAX) &&
			    (abs(_gMutualICDeltaC[i]) < MSG28XX_IIR_MAX)) {
...
static s32 _DrvMpTestItoShortTestMsg28xxJudge(u8 nItemID, /*s8 pNormalTestResult[][2],*/
		u16 pTestPinMap[][13], u16 *pTestPinCount)
{
	s32 nRetVal = 0;
	...
	*pTestPinCount = _DrvMpTestItoShortTestMsg28xxReadTestPins(nItemID, nTestPins);
	...
	for (i = 0; i < *pTestPinCount; i++) {
		...
		if (_DrvMpTestMutualICCheckValueInRange(_gMutualICResult[i + (nItemID - 1) * 13],
				MSG28XX_SHORT_VALUE, -1000) == 0) {
...
static s32 _DrvMpTestItoShortTestMsg28xxOnCellJudge(u8
		nItemID, /*s8 pNormalTestResult[][2],*/ u16 pTestPinMap[][13], s8 *TestFail,
		u16 *pTestPinCount)
{
	s32 nRetVal = 0;
	...
	*pTestPinCount = _DrvMpTestItoShortTestMsg28xxReadTestPins(nItemID, aTestPins);
	...
	for (i = 0; i < *pTestPinCount; i++) {
		...
		if (_DrvMpTestMutualICCheckValueInRange(_gMutualICResult[i + (nItemID - 1) * 13],
				_gMsg28xx_SHORT_VALUE, -_gMsg28xx_SHORT_VALUE) == 0) {
			DBG(&g_I2cClient->dev, "*** Msg28xx Short Test# ShortTestMsg28xxJudge failed! ***\n");
			TestFail[nItemID] = 1;
			nRetVal = -1;
		}
...
static void _DrvMpTestIcPinShortTestMsg28xxOnCellChangeANASetting(void)
{
	...
	memcpy(_gMsg28xxMuxMem_20_3E_1_Settings, _gMsg28xx_SHORT_IcPIN_MUX_MEM_1_Settings,
	       sizeof(_gMsg28xxMuxMem_20_3E_1_Settings));
	memcpy(_gMsg28xxMuxMem_20_3E_2_Settings, _gMsg28xx_SHORT_IcPIN_MUX_MEM_2_Settings,
	       sizeof(_gMsg28xxMuxMem_20_3E_2_Settings));
	memcpy(_gMsg28xxMuxMem_20_3E_3_Settings, _gMsg28xx_SHORT_IcPIN_MUX_MEM_3_Settings,
	       sizeof(_gMsg28xxMuxMem_20_3E_3_Settings));
	memcpy(_gMsg28xxMuxMem_20_3E_4_Settings, _gMsg28xx_SHORT_IcPIN_MUX_MEM_4_Settings,
	       sizeof(_gMsg28xxMuxMem_20_3E_4_Settings));
	memcpy(_gMsg28xxMuxMem_20_3E_5_Settings, _gMsg28xx_SHORT_IcPIN_MUX_MEM_5_Settings,
	       sizeof(_gMsg28xxMuxMem_20_3E_5_Settings));
	...
	for (nMappingItem = 0; nMappingItem < 6; nMappingItem++) {
		...
		for (i = 0; i < 8; i++) {
			if (nMappingItem == 0 && nChipVer == 0x0) {
				...
				RegSet16BitValue(0x218C, _gMsg28xxMuxMem_20_3E_0_Settings[2 * i + 1]);
				DBG(&g_I2cClient->dev,
				    "_gMsg28xxMuxMem_20_3E_0_Settings[%d] = %x, _gMsg28xxMuxMem_20_3E_0_Settings[%d] = %x\n",
				    2 * i, _gMsg28xxMuxMem_20_3E_0_Settings[2 * i], 2 * i + 1,
				    _gMsg28xxMuxMem_20_3E_0_Settings[2 * i + 1]);
			}
			...
			if ((nMappingItem == 1 && nChipVer == 0x0) || (nMappingItem == 0 && nChipVer != 0x0)) {
				...
				RegSet16BitValue(0x218C, _gMsg28xxMuxMem_20_3E_1_Settings[2 * i + 1]);
				DBG(&g_I2cClient->dev,
				    "_gMsg28xxMuxMem_20_3E_1_Settings[%d] = %x, _gMsg28xxMuxMem_20_3E_1_Settings[%d] = %x\n",
				    2 * i, _gMsg28xxMuxMem_20_3E_1_Settings[2 * i], 2 * i + 1,
				    _gMsg28xxMuxMem_20_3E_1_Settings[2 * i + 1]);
			}
			...
			if ((nMappingItem == 2 && nChipVer == 0x0) || (nMappingItem == 1 && nChipVer != 0x0)) {
				...
				RegSet16BitValue(0x218C, _gMsg28xxMuxMem_20_3E_2_Settings[2 * i + 1]);
				DBG(&g_I2cClient->dev,
				    "_gMsg28xxMuxMem_20_3E_2_Settings[%d] = %x, _gMsg28xxMuxMem_20_3E_2_Settings[%d] = %x\n",
				    2 * i, _gMsg28xxMuxMem_20_3E_2_Settings[2 * i], 2 * i + 1,
				    _gMsg28xxMuxMem_20_3E_2_Settings[2 * i + 1]);
			}
			...
			if ((nMappingItem == 3 && nChipVer == 0x0) || (nMappingItem == 2 && nChipVer != 0x0)) {
				...
				RegSet16BitValue(0x218C, _gMsg28xxMuxMem_20_3E_3_Settings[2 * i + 1]);
				DBG(&g_I2cClient->dev,
				    "_gMsg28xxMuxMem_20_3E_3_Settings[%d] = %x, _gMsg28xxMuxMem_20_3E_3_Settings[%d] = %x\n",
				    2 * i, _gMsg28xxMuxMem_20_3E_3_Settings[2 * i], 2 * i + 1,
				    _gMsg28xxMuxMem_20_3E_3_Settings[2 * i + 1]);
			}
			...
			if ((nMappingItem == 4 && nChipVer == 0x0) || (nMappingItem == 3 && nChipVer != 0x0)) {
				...
				RegSet16BitValue(0x218C, _gMsg28xxMuxMem_20_3E_4_Settings[2 * i + 1]);
				DBG(&g_I2cClient->dev,
				    "_gMsg28xxMuxMem_20_3E_4_Settings[%d] = %x, _gMsg28xxMuxMem_20_3E_4_Settings[%d] = %x\n",
				    2 * i, _gMsg28xxMuxMem_20_3E_4_Settings[2 * i], 2 * i + 1,
				    _gMsg28xxMuxMem_20_3E_4_Settings[2 * i + 1]);
			}
			...
			if ((nMappingItem == 5 && nChipVer == 0x0) || (nMappingItem == 4 && nChipVer != 0x0)) {
				...
				RegSet16BitValue(0x218C, _gMsg28xxMuxMem_20_3E_5_Settings[2 * i + 1]);
				DBG(&g_I2cClient->dev,
				    "_gMsg28xxMuxMem_20_3E_5_Settings[%d] = %x, _gMsg28xxMuxMem_20_3E_5_Settings[%d] = %x\n",
				    2 * i, _gMsg28xxMuxMem_20_3E_5_Settings[2 * i], 2 * i + 1,
				    _gMsg28xxMuxMem_20_3E_5_Settings[2 * i + 1]);
			}
			...
			if (nMappingItem == 5 && nChipVer != 0x0) {
				...
				RegSet16BitValue(0x218C, _gMsg28xxMuxMem_20_3E_6_Settings[2 * i + 1]);
				DBG(&g_I2cClient->dev,
				    "_gMsg28xxMuxMem_20_3E_6_Settings[%d] = %x, _gMsg28xxMuxMem_20_3E_6_Settings[%d] = %x\n",
				    2 * i, _gMsg28xxMuxMem_20_3E_6_Settings[2 * i], 2 * i + 1,
				    _gMsg28xxMuxMem_20_3E_6_Settings[2 * i + 1]);
			}
		}
	}
}
...
static s32 _DrvMpTestIcPinShortTestMsg28xxOnCellReadMapping(u16 testPin_data[][13])
{
	...
	for (nMappingItem = 1; nMappingItem < 6; nMappingItem++) {
		...
		for (i = 0;
		     i < sizeof(_gMsg28xxMuxMem_20_3E_1_Settings) / sizeof(
			     _gMsg28xxMuxMem_20_3E_1_Settings[0]); i++) {
			for (j = 0; j < 4; j++) {
				if (nMappingItem == 1) {
...
static s32 _DrvMpTestIcPinShortTestMsg28xxOnCellShortTestJudge(u16 nItemID,
		u16 pTestPinMap[][13], s8 *TestFail)
{
	...
	for (i = 0; i < ARRAY_SIZE(pTestPinMap); i++) {
	...
	for (i = 0; i < count_test_pin; i++) {
		if (_DrvMpTestMutualICCheckValueInRange(_gMutualICDeltaC[i + (nItemID - 1) * 13],
				_gMsg28xx_ICPIN_SHORT, -_gMsg28xx_ICPIN_SHORT) == 0) {
			TestFail[nItemID] = 1;
		}
	}
	...
	for (i = 0; i < MUTUAL_IC_MAX_CHANNEL_NUM; i++) {
		...
		if (_gMsg28xx_SENSE_PAD_PIN_MAPPING[i] != 0) {
			DBG(&g_I2cClient->dev, "pICPinChannel[%d] = %d, IC Pin P[%d] = %dM, deltaC = %d\n", i,
			    _gMsg28xx_SENSE_PAD_PIN_MAPPING[i], _gMsg28xx_SENSE_PAD_PIN_MAPPING[i], senseR[i],
			    _gIcPinShortResultData[i]);

			if (senseR[i] < thrs) {
				_gIcPinShortFailChannel[i] = _gMsg28xx_SENSE_PAD_PIN_MAPPING[i];
				DBG(&g_I2cClient->dev, "IC Pin Fail P%d = %dM", _gMsg28xx_SENSE_PAD_PIN_MAPPING[i],
				    senseR[i]);
				count++;
				nRet = -1;
			}
...
_RETRY_SHORT:
	...
	for (i = 1; i < 6; i++) {
		...
		for (j = 0; j < 13; j++) {
			/* max 13 AFE */
			if (((i - 1) * 13 + j) <
			    MUTUAL_IC_MAX_CHANNEL_NUM) {	/* prevent heap corruption detected */
	...
	for (nTestItem = 1; nTestItem < nTestItemLoop; nTestItem++) {
		...
		if (_DrvMpTestIcPinShortTestMsg28xxOnCellShortTestJudge(nTestItem, /*aNormalTestResult,*/
				aTestPinMap, aNormalTestFail) < 0) {
...
_DrvMpTestItoShortTestMsg28xxOnCellCovertRValue(_gMutualICDeltaC[i + (nTestItem - 1) * 13]);
					_gIcPinShortResultData[aTestPinMap[nTestItem][i] - 1]
						= _gMutualICDeltaC[i + (nTestItem - 1) * 13];
					DBG(&g_I2cClient->dev, "pSenseR[%d]={%d}, _gDeltaC[{%d}]={%d}",
					    aTestPinMap[nTestItem][i] - 1, pSenseR[aTestPinMap[nTestItem][i] - 1],
					    i + (nTestItem - 1) * 13, _gMutualICDeltaC[i + (nTestItem - 1) * 13]);
				} else {
					nRetVal = -1;
...
_RETRY_SHORT:
	...
	for (nTestItem = 1; nTestItem < nTestItemLoop; nTestItem++) {
		DBG(&g_I2cClient->dev, "*** Short test item %d ***\n", nTestItem);
		if (_DrvMpTestItoShortTestMsg28xxOnCellJudge(nTestItem, /*aNormalTestResult,*/
				aTestPinMap, aNormalTestFail, &nTestPinNum) < 0) {
			DBG(&g_I2cClient->dev, "*** Msg28xx Short Test# Item %d is failed! ***\n", nTestItem);
			nRetVal = -1;
			goto ITO_TEST_END;
		}

		if (nTestItem == 1 || nTestItem == 2 || (nTestItem == 5 &&
				_gMsg28xx_SHORT_TEST_5_TYPE == 1)) {
			...
			for (i = 0; i < nTestPinNum; i++) {
				for (j = 0; j < _gMutualICSenseLineNum; j++) {
					...
					pSenseR[j] =
_DrvMpTestItoShortTestMsg28xxOnCellCovertRValue(_gMutualICResult[i + (nTestItem - 1) * 13]);
					DBG(&g_I2cClient->dev, "senseR[%d] = %d, _gDeltaC[%d] = %d\n", j, pSenseR[j],
					i + (nTestItem - 1) * 13, _gMutualICResult[i + (nTestItem - 1) * 13]);
		...
		if (_DrvMpTestMutualICCheckValueInRange(_gMutualICResult[i + (nTestItem - 1) * 13],
					_gMsg28xx_SHORT_VALUE, -_gMsg28xx_SHORT_VALUE) == 0) {
		...
		if (nTestItem == 3 || nTestItem == 4 || (nTestItem == 5 &&
				_gMsg28xx_SHORT_TEST_5_TYPE == 2)) {
	...
	_DrvMpTestItoShortTestMsg28xxOnCellCovertRValue(_gMutualICResult[i + (nTestItem - 1) * 13]);
				DBG(&g_I2cClient->dev, "driveR[%d] = %d, _gDeltaC[%d] = %d\n", j, pDriveR[j],
						    i + (nTestItem - 1) * 13,
						    _gMutualICResult[i + (nTestItem - 1) * 13]);
						_gItoShortRData[_gMutualICSenseLineNum + j] = pDriveR[j];
						_gItoShortResultData[_gMutualICSenseLineNum + j]
						= _gMutualICResult[i + (nTestItem - 1) * 13];
			...
			if (_DrvMpTestMutualICCheckValueInRange(_gMutualICResult[i + (nTestItem - 1) * 13],
						_gMsg28xx_SHORT_VALUE, -_gMsg28xx_SHORT_VALUE) == 0) {
						_gItoShortFailChannel[_gMutualICSenseLineNum + j] =
	...
	_DrvMpTestItoShortTestMsg28xxOnCellCovertRValue(_gMutualICResult[i + (nTestItem - 1) * 13]);
				_gItoShortRData[_gMutualICSenseLineNum + _gMutualICDriveLineNum + j] = pGRR[j];
					_gItoShortResultData[_gMutualICSenseLineNum + _gMutualICDriveLineNum + j] =
						_gMutualICResult[i + (nTestItem - 1) * 13];
			...
			if (_DrvMpTestMutualICCheckValueInRange(_gMutualICResult[i + (nTestItem - 1) * 13],
					_gMsg28xx_SHORT_VALUE, -_gMsg28xx_SHORT_VALUE) == 0) {
				_gItoShortFailChannel[_gMutualICSenseLineNum + _gMutualICDriveLineNum + j] =
								(u32)aTestPinMap[nTestItem][i];
						}
					}
				}
...
_RETRY_SHORT:
	...
	for (nTestItem = 1; nTestItem < nTestItemLoop; nTestItem++) {
		DBG(&g_I2cClient->dev, "*** Short test item %d ***\n", nTestItem);
		if (_DrvMpTestItoShortTestMsg28xxJudge(nTestItem, /*aNormalTestResult,*/ aTestPinMap,
						       &nTestPinNum) < 0) {
			DBG(&g_I2cClient->dev, "*** Msg28xx Short Test# Item %d is failed! ***\n", nTestItem);
			nRetVal = -1;
			goto ITO_TEST_END;
		...
		if (nTestItem == 1 || nTestItem == 2 || (nTestItem == 5 &&
			...
			for (i = 0; i < nTestPinNum; i++) {
				for (j = 0; j < _gMutualICSenseLineNum; j++) {
					if (aTestPinMap[nTestItem][i] == pPad2Sense[j]) {
						_gMutualICSenseR[j] = _gMutualICResult[i + (nTestItem - 1) * 13];
					}
				}
			}
		}
		...
		if (nTestItem == 3 || nTestItem == 4 || (nTestItem == 5 &&
			...
			for (i = 0; i < nTestPinNum; i++) {
				for (j = 0; j < _gMutualICDriveLineNum; j++) {
					if (aTestPinMap[nTestItem][i] == pPad2Drive[j]) {
						_gMutualICDriveR[j] = _gMutualICResult[i + (nTestItem - 1) *
										       13];
					}
				}
			}
		...
		if (nTestItem == 5 && _gMsg28xx_SHORT_TEST_5_TYPE == 3) {
			for (i = 0; i < nTestPinNum; i++) {
				for (j = 0; j < _gMsg28xx_GR_NUM; j++) {
					if (aTestPinMap[nTestItem][i] == aPad2GR[j]) {
						_gMutualICGRR[j] = _gMutualICResult[i + (nTestItem - 1) *
										    13];
					}
				}
			}
	...
	_DrvMpTestMutualICDebugShowArray(_gMutualICResult,
					 _gMutualICSenseLineNum + _gMutualICDriveLineNum, -32, 10, 8);
	/* for (i = 0; i < (_gMutualICSenseLineNum + _gMutualICDriveLineNum - 1); i++) */
	for (i = 0; i < (_gMutualICSenseLineNum + _gMutualICDriveLineNum); i++) {
		if (aResultTemp[i] >
		    MSG28XX_SHORT_VALUE) {  /* change comparison way because float computing in driver is prohibited */
			_gMutualICTestFailChannel[i] = 1;
			_gTestFailChannelCount++;
			nRetVal = -1;
		} else {
...
static s32 _DrvMpTestItoWaterProofTestMsg28xxTriggerWaterProofOneShot(s16 *pResultData,
		u32 nDelay)
{
	u16 nAddr = 0x5000, nAddrNextSF = 0x1A4;
	u16 nSF = 0, nAfeOpening = 0, nDriOpening = 0;
	...
	if (_gMsg28xx_PATTERN_TYPE == 1) { /* for short test */
		...
		for (i = 0; i < 8; i++) {
		   ...
		   _gMsg28xx_PATTERN_TYPE == 4) { /* for open test */
		/* s16 nOpenResultData[nSF * nAfeOpening][nDriOpening]; */
...
static s32 _DrvMpTestItoWaterProofTesMsg28xxtGetWaterProofOneShotRawIIR(s16 *pRawDataWP,
		u32 nDelay)
{
	return _DrvMpTestItoWaterProofTestMsg28xxTriggerWaterProofOneShot(pRawDataWP, nDelay);
}

static s32 _DrvMpTestItoWaterProofTestMsg28xxGetDeltaCWP(s32 *pTarget, s8 nSwap,
		u32 nDelay)
{
	s16 nRawDataWP[12] = {0};
	s16 i;
	...
	if (_DrvMpTestItoWaterProofTesMsg28xxtGetWaterProofOneShotRawIIR(nRawDataWP,
			nDelay) < 0) {
		DBG(&g_I2cClient->dev, "*** Msg28xx Open Test# GetMutualOneShotRawIIR failed! ***\n");
		return -EPERM;
	}
...
static s32 _DrvMpTestMsg28xxItoWaterProofTest(u32 nDelay)
{
	...
	if (_DrvMpTestItoWaterProofTestMsg28xxGetDeltaCWP(_gMutualICDeltaCWater, -1,
			nDelay) < 0) {
...
static s32 _DrvMpTestMsg28xxItoWaterProofTestEntry(void)
{
	...
	for (i = 0; i < _gMutualICWaterProofNum; i++) {
		if (_gMutualICResultWater[i] >
		    MSG28XX_WATER_VALUE) {  /* change comparison way because float computing in driver is prohibited */
			_gMutualICTestFailChannel[i] = 1;
			_gTestFailChannelCount++;
			nRetVal = -1;
		} else {
		...
		if (_gTestRetryCount > 0) {
			...
			if (((g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX) &&
				...
				_gCtpMpTestStatus = ITO_TEST_FAIL;
				    ...
				    (nRetVal == ITO_TEST_GET_TP_TYPE_ERROR))
				   || ((g_ChipType == CHIP_TYPE_MSG26XXM ||
					g_ChipType == CHIP_TYPE_MSG28XX/* || g_ChipType == CHIP_TYPE_MSG58XXA*/) &&
				       (nRetVal == -2))) {
...
void DrvMpTestGetTestFailChannel(ItoTestMode_e eItoTestMode, u8 *pFailChannel,
				 u32 *pFailChannelCount)
{
#if defined(CONFIG_ENABLE_CHIP_TYPE_MSG21XXA)
	|| defined(CONFIG_ENABLE_CHIP_TYPE_MSG22XX)
	|| defined(CONFIG_ENABLE_CHIP_TYPE_MSG26XXM)
	|| defined(CONFIG_ENABLE_CHIP_TYPE_MSG28XX)
	u32 i;
#endif
		...
		if (eItoTestMode == ITO_TEST_MODE_OPEN_TEST) {
			...
			for (i = 0; i < SELF_IC_MAX_CHANNEL_NUM; i++) {
				...
				if (_gSelfICDataFlag2[i] == 1) {
					pDataLog[i * 4 + SELF_IC_MAX_CHANNEL_NUM * 4] =
						1; /* indicate it is a on-use channel number */
				} else {
					pDataLog[i * 4 + SELF_IC_MAX_CHANNEL_NUM * 4] =
						0; /* indicate it is a non-use channel number */
				}

				if (_gSelfICRawData2[i] >= 0) {
					pDataLog[(i * 4 + 1) + SELF_IC_MAX_CHANNEL_NUM * 4] = 0;
				} else {
					pDataLog[(i * 4 + 1) + SELF_IC_MAX_CHANNEL_NUM * 4] = 1;
				}
			...
			*pLength = SELF_IC_MAX_CHANNEL_NUM * 8;
			...
			for (i = 0; i < SELF_IC_MAX_CHANNEL_NUM; i++) {
				...
				if (_gSelfICDataFlag3[i] == 1) {
					pDataLog[i * 4 + SELF_IC_MAX_CHANNEL_NUM * 8] =
						1; /* indicate it is a on-use channel number */
				} else {
					pDataLog[i * 4 + SELF_IC_MAX_CHANNEL_NUM * 8] =
						0; /* indicate it is a non-use channel number */
				...
				if (_gSelfICRawData3[i] >= 0) {
					pDataLog[(i * 4 + 1) + SELF_IC_MAX_CHANNEL_NUM * 8] = 0;
				} else {
					pDataLog[(i * 4 + 1) + SELF_IC_MAX_CHANNEL_NUM * 8] = 1;
				}
			...
			if (_gSelfICIsEnable2R) {
				for (i = 0; i < SELF_IC_MAX_CHANNEL_NUM; i++) {
					...
					if (_gSelfICDataFlag4[i] == 1) {
						pDataLog[i * 4 + SELF_IC_MAX_CHANNEL_NUM * 12] =
							1; /* indicate it is a on-use channel number */
					} else {
						pDataLog[i * 4 + SELF_IC_MAX_CHANNEL_NUM * 12] =
							0; /* indicate it is a non-use channel number */
					...
					if (_gSelfICRawData4[i] >= 0) {
						pDataLog[(i * 4 + 1) + SELF_IC_MAX_CHANNEL_NUM * 12] = 0;
					} else {
						pDataLog[(i * 4 + 1) + SELF_IC_MAX_CHANNEL_NUM * 12] = 1;
					}
		...
		if (eItoTestMode == ITO_TEST_MODE_OPEN_TEST) {
			...
			for (j = 0; j < _gMutualICDriveLineNum; j++) {
				for (i = 0; i < _gMutualICSenseLineNum; i++) {
					...
					pDataLog[k * 5 + 1] =
						(_gMutualICResult[i * _gMutualICDriveLineNum + j] >> 24) & 0xFF;
					pDataLog[k * 5 + 2] =
						(_gMutualICResult[i * _gMutualICDriveLineNum + j] >> 16) & 0xFF;
					pDataLog[k * 5 + 3] =
						(_gMutualICResult[i * _gMutualICDriveLineNum + j] >> 8) & 0xFF;
					pDataLog[k * 5 + 4] =
						(_gMutualICResult[i * _gMutualICDriveLineNum + j]) & 0xFF;
			...
			*pLength = k * 5;
			...
			for (i = 0; i < _gMutualICWaterProofNum; i++) {
				...
				pDataLog[k * 5 + 1] =
						(_gMutualICResultWater[i] >> 24) & 0xFF;
				pDataLog[k * 5 + 2] =
						(_gMutualICResultWater[i] >> 16) & 0xFF;
				pDataLog[k * 5 + 3] =
						(_gMutualICResultWater[i] >> 8) & 0xFF;
				pDataLog[k * 5 + 4] =
						(_gMutualICResultWater[i]) & 0xFF;
				k++;
			}
...
void DrvMpTestGetTestLogAll(u8 *pDataLog, u32 *pLength)
{
	...
	if (_gMsg28xx_PATTERN_TYPE == 5) {
		...
		*pLength = 0;
		...
		if (g_IsInMpTest != 0) {
			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "Test is still running!!!\n");
		} else {
			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "\n");

			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "Device Driver Version : %s\n",
						      DEVICE_DRIVER_RELEASE_VERSION);

			DrvIcFwLyrGetCustomerFirmwareVersion(&nMajor, &nMinor, &pVersion);
			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "Main Block FW Version : %d.%03d\n",
						      nMajor, nMinor);
...
DrvIcFwLyrGetCustomerFirmwareVersionByDbBus(EMEM_INFO, &nMajor, &nMinor, &pVersion);
			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "Info Block FW Version : %d.%03d\n", nMajor, nMinor);
			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "SupportIC : %d\n", _gMsg28xx_SupportIC);

			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength,	"DC_Range=%d\n", _gMsg28xx_DC_RANGE);

			*pLength = *pLength + snprintf(pDataLog + *pLength,
			all_length - *pLength, "DC_Ratio_1000=%d\n", _gMsg28xx_DC_Ratio_1000);

			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "DC_Border_Ratio_1000=%d\n",
				_gMsg28xx_DC_Border_Ratio_1000);
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength,
			...
			pItem = "Golden";
			*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "%10s", pItem);

			for (i = 0; i < _gMsg28xx_DRIVE_NUM; i++) {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
					all_length - *pLength, "%5s%2d", pRowStr, i + 1);
			}

			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");

			for (i = 0; i < _gMsg28xx_SENSE_NUM * _gMsg28xx_DRIVE_NUM; i++) {
				if ((i % _gMsg28xx_DRIVE_NUM) == 0) {
					*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "%8s%2d", pColStr, (i / _gMsg28xx_DRIVE_NUM) + 1);
					*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "%7d", _gMutualICOnCellOpenTestGoldenChannel[i]);
				} else if ((i % _gMsg28xx_DRIVE_NUM) == (_gMsg28xx_DRIVE_NUM - 1)) {
					*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "%7d\n", _gMutualICOnCellOpenTestGoldenChannel[i]);
				} else {
					*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "%7d", _gMutualICOnCellOpenTestGoldenChannel[i]);
				}
		...
		*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			for (i = 0; i < _gMsg28xx_SENSE_NUM * _gMsg28xx_DRIVE_NUM; i++) {
				...
				all_length - *pLength, "%8s%2d", pColStr, (i / _gMsg28xx_DRIVE_NUM) + 1);
					...
					*pLength = *pLength + snprintf(pDataLog + *pLength,
			all_length - *pLength, "%7d", _gMutualICOnCellOpenTestGoldenChannelMax[i]);
				} else if ((i % _gMsg28xx_DRIVE_NUM) == (_gMsg28xx_DRIVE_NUM - 1)) {
					*pLength = *pLength + snprintf(pDataLog + *pLength,
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			for (i = 0; i < _gMsg28xx_SENSE_NUM * _gMsg28xx_DRIVE_NUM; i++) {
				if ((i % _gMsg28xx_DRIVE_NUM) == 0) {
					*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "%8s%2d", pColStr,
								      (i / _gMsg28xx_DRIVE_NUM) + 1);
			...
			if (_gMutualICOnCellOpenTestResult[0] == 0) {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "DeltaC_Result:PASS\n");
			} else {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "DeltaC_Result:FAIL\n");
				pItem = "Fail Channel:  ";
				*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "%s", pItem);
				for (i = 0; i < _gMsg28xx_SENSE_NUM * _gMsg28xx_DRIVE_NUM; i++) {
					if (_gNormalTestFailCheckDeltac[i] == MSG28XX_PIN_NO_ERROR) {
						continue;
					}
					*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "D%d.S%d",
								      _gNormalTestFailCheckDeltac[i] % 100,
								      _gNormalTestFailCheckDeltac[i] / 100);
					*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "    ");
				}
		...
		*pLength = *pLength + snprintf(pDataLog + *pLength,
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			if (_gMutualICOnCellOpenTestResult[1] == 0) {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "Ratio_Result:PASS\n");
			} else {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "Ratio_Result:FAIL\n");
				pItem = "Fail Channel:  ";
				*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "%s", pItem);
				for (i = 0; i < _gMsg28xx_SENSE_NUM * _gMsg28xx_DRIVE_NUM; i++) {
					...
					*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "D%d.S%d",
								      _gNormalTestFailCheckRatio[i] % 100,
									_gNormalTestFailCheckRatio[i] / 100);
					*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "    ");
				}
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength, "ShortValue=%d\n",
							_gMsg28xx_SHORT_VALUE);
			*pLength = *pLength + snprintf(pDataLog + *pLength, "ICPinShort=%d\n",
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength,
			...
			for (i = 0; i < MUTUAL_IC_MAX_CHANNEL_NUM; i++) {
				if (_gMsg28xx_SENSE_PAD_PIN_MAPPING[i] != 0) {
					*pLength = *pLength + snprintf(pDataLog + *pLength,
									all_length - *pLength, "%5s%2d", pPinStr,
									_gMsg28xx_SENSE_PAD_PIN_MAPPING[i]);
				}
			}
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength,
			...
			for (i = 0; i < MUTUAL_IC_MAX_CHANNEL_NUM; i++) {
				if (_gMsg28xx_SENSE_PAD_PIN_MAPPING[i] != 0) {
					*pLength = *pLength + snprintf(pDataLog + *pLength,
					all_length - *pLength, "%6dM", _gIcPinShortSenceR[i]);
				}
			}
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength,
			...
			for (i = 0; i < MUTUAL_IC_MAX_CHANNEL_NUM; i++) {
				if (_gMsg28xx_SENSE_PAD_PIN_MAPPING[i] != 0) {
					*pLength = *pLength + snprintf(pDataLog + *pLength,
					all_length - *pLength, "%7d", _gIcPinShortResultData[i]);
				}
			}
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			if (_gIcPinShortCheckFail == 0) {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
							all_length - *pLength, "ICPin Short Test:PASS\n");
			} else {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
							all_length - *pLength, "ICPin Short Test:FAIL\n");
				pItem = "Fail Channel:  ";
				*pLength = *pLength + snprintf(pDataLog + *pLength,
							all_length - *pLength, "%s", pItem);
				for (i = 0; i < MUTUAL_IC_MAX_CHANNEL_NUM; i++) {
					if (_gIcPinShortFailChannel[i] != 0) {
						*pLength = *pLength + snprintf(pDataLog + *pLength,
						all_length - *pLength, "P%d", _gIcPinShortFailChannel[i]);
						*pLength = *pLength + snprintf(pDataLog + *pLength,
							all_length - *pLength, "    ");
					}
				}
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			for (i = 0; i < _gMsg28xx_SENSE_NUM; i++) {
				if ((i % 10) == 0) {
					...
					*pLength = *pLength + snprintf(pDataLog + *pLength,
							all_length - *pLength, "%8s%2d", pColStr, i);
				}

				*pLength = *pLength + snprintf(pDataLog + *pLength,
							all_length - *pLength, "%6dM", _gItoShortRData[i]);
			}
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			for (i = 0; i < _gMsg28xx_DRIVE_NUM; i++) {
				if ((i % 10) == 0) {
					...
					*pLength = *pLength + snprintf(pDataLog + *pLength,
								all_length - *pLength, "%8s%2d", pRowStr, i);
				}

				*pLength = *pLength + snprintf(pDataLog + *pLength,
								all_length - *pLength, "%6dM",
								_gItoShortRData[i + _gMsg28xx_SENSE_NUM]);
			}
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			...
			for (i = 0; i < _gMsg28xx_SENSE_NUM; i++) {
				...
				*pLength = *pLength + snprintf(pDataLog + *pLength,
				all_length - *pLength, "%7d", _gItoShortResultData[i]);
			}
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
			for (i = 0; i < _gMsg28xx_DRIVE_NUM; i++) {
				if ((i % 10) == 0) {
				...
				*pLength = *pLength + snprintf(pDataLog + *pLength,
								all_length - *pLength, "%7d",
								_gItoShortResultData[i + _gMsg28xx_SENSE_NUM]);
			}
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");

			if (_gItoShortCheckFail == 0) {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
								all_length - *pLength, "ITO Short Test:PASS\n");
			} else {
				*pLength = *pLength + snprintf(pDataLog + *pLength,
								all_length - *pLength, "ITO Short Test:FAIL\n");
			}
			*pLength = *pLength + snprintf(pDataLog + *pLength, all_length - *pLength, "\n");
