...
#define DEVICE_DRIVER_RELEASE_VERSION   ("5.7.0.0")
...
/*
 * Note.
 * The below compile option is used to enable the specific device driver code handling for distinct smart phone
 *developer platform.
...
/*
 * Note.
 * The below compile option is used to enable the specific device driver code handling to make sure main board can
 * supply power to touch ic for some specific BB chip of MTK(EX. MT6582)/SPRD(EX. SC7715)/QCOM(EX. MSM8610).
 * By default, this compile option is disabled.
 */
...
/*
 * Note.
 * The below compile option is used to distinguish different workqueue scheduling mechanism when firmware report
 * finger touch to device driver by IRQ interrupt.
...
/*
 * Note.
 * The below compile option is used to enable the specific device driver code handling when touch panel support
 * virtual key(EX. Menu, Home, Back, Search).
...
/*
 * Note.
 * Since specific MTK BB chip report virtual key touch by using coordinate instead of key code, the below compile
 * option is used to enable the code handling for reporting key with coordinate.
 * By default, this compile option is disabled.
 */
 ...
 * Since I2C transaction length limitation for some specific MTK BB chip(EX. MT6589/MT6572/...) or QCOM BB chip,
 * the debug mode finger touch data of MSG26XXM/MSG28XX/MSG58XX/MSG58XXA
...
/*
 * Note.
 * The below compile option shall be used when compile option CONFIG_ENABLE_ITO_MP_TEST
 * or CONFIG_UPDATE_FIRMWARE_BY_SW_ID is enabled.
...
/*
 * Note.
 * The below compile option is used to enable charger detection for notifying the charger
 * plug in/plug out status to touch firmware.
 * By default, this compile option is disabled.
 */
 ...
 * Method 1. Require the new ESD check command(CmdId:0x55) support from firmware which is currently
 * implemented for MSG22XX only. So default is not supported.
...
/*
 * Note.
 * The below compile option is used to enable/disable Type A/Type B multi-touch protocol for reporting
 * touch point/key to Linux input sub-system.
 ...
 * By the way, force touch is supported for MSG28XX/MSG58XX/MSG58XXA only, but is not supported for
 MSG26XXM/MSG21XXA/MSG22XX yet.
 * By default, this compile option is disabled.
 */
...
/*
 * Note.
 * The below two compile option is used to enable update firmware with 8 byte or 32 byte each time for
 * MSG28XX/MSG58XX/MSG58XXA.
 * If the below two compile option is disabled, then update firmware with 128 byte each time for
 * MSG28XX/MSG58XX/MSG58XXA.
...
#define MAX_DEBUG_COMMAND_ARGUMENT_NUM      (4)
 /* 130KB. The size shall be large enough for stored any kind firmware size of MSG21XXA(33KB)
 /MSG22XX(48.5KB)/MSG26XXM(40KB)/MSG28XX(130KB)/MSG58XX(130KB)/MSG58XXA(130KB). */
...
extern int global_eSwId;
extern int global_nMajor;
extern int global_nMinor;
