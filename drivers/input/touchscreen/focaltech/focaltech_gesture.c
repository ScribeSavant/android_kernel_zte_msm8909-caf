...
{
	input_set_capability(input_dev, EV_KEY, /*KEY_POWER*/KEY_WAKEUP);
...
static void fts_check_gesture(struct input_dev *input_dev,int gesture_id)
{
	switch(gesture_id)
	{
	        ...
	        case GESTURE_DOUBLECLICK:
	                input_report_key(input_dev, /*KEY_GESTURE_U*/KEY_WAKEUP, 1);
	                input_sync(input_dev);
	                input_report_key(input_dev, /*KEY_GESTURE_U*/KEY_WAKEUP, 0);
	                input_sync(input_dev);
	                break;
	        case GESTURE_O:
