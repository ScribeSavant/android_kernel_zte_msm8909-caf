...
{
	int rc = 0;
	pr_debug("%s enter! i2c_addr = 0x%x.\n",__func__,i2c_addr);
...
static bool imx219_otp_read_awb(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	awbRB <<= 8;
	awbRB += buf[5];
	pr_info("%s OTP data are awbRG=%x, awbBG=%x, awbRB=%x!!!\n",
...
static bool imx219_otp_read_lsc(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	pr_info("%s LCS[0]= %x,LSC[244] = %x  LSC[245]=%x,LSC[279]=%d\n",__func__,
...
static bool imx219_otp_read_vcm(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if((imx219_otp_flag & IMX219_OTP_VCM_READ) == IMX219_OTP_VCM_READ) { //OTP data is read all ready
		pr_debug("%s OTP VCM data is read all ready!!!\n",__func__);
		return true;
	}
	...
	if((start_code != end_code) &&(end_code>start_code) && start_code != 0 && end_code != 0) {
		...
		pr_info("%s imx219_vcm_start= 0x%x, imx219_vcm_end = 0x%x \n",__func__, imx219_vcm_start,imx219_vcm_end);

	} else {//Abnormal branch deal
		imx219_vcm_start = 0;
		imx219_vcm_end = 0;
		pr_err("%s VCM OTP data is worng! imx219_vcm_start= 0x%x, imx219_vcm_end = 0x%x \n",__func__, imx219_vcm_start,imx219_vcm_end);
		return false;
	}
...
static bool imx219_get_otp_from_sensor(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	if((imx219_otp_flag & IMX219_OTP_FAIL_FLAG) == IMX219_OTP_FAIL_FLAG) {//Just check OTP once whether success or not
		...
		pr_info("%s OTP has been read success already!\n",__func__);
		return true;
	}
	...
	if(false == retVal){
		pr_err("%s imx219_otp_read_id(s_ctrl) failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if(false == retVal){
		pr_err("%s imx219_otp_read_awb(s_ctrl) failed!\n",__func__);
		goto OTP_FAIL;
	}
	...
	if(false == retVal){
		pr_err("%s imx219_otp_read_vcm(s_ctrl) failed!\n",__func__);
		goto OTP_FAIL;
	}
...
OTP_FAIL:
	...
	pr_info("%s imx219_otp_flag=0x%x \n", __func__,imx219_otp_flag);
	s_ctrl->hw_otp_check_flag.mmi_otp_check_flag  = tmp_mmi_otp_flag;
	pr_info("%s imx219_mmi_otp_flag = 0x%x\n",__func__, s_ctrl->hw_otp_check_flag.mmi_otp_check_flag);
	return false;
}
...
int imx219_otp_func(struct msm_sensor_ctrl_t * s_ctrl, int index)
{
	...
	pr_info("%s,IMX219 OTP read and set OK.\n", __func__);
	return rc;
}